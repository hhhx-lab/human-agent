...
`strict_unknown_fields` 的默认值应为 true。真实数字生命系统里，未知字段不是无害扩展；它可能是绕过 protected core、scope graph 或 coexistence boundary control overlay 的入口。
...
L1 不是充分条件。真实数字生命系统最危险的错误往往形状正确但语义错误，比如 deleted 对象仍有可恢复 locator，或 sandbox hypothesis 以合法字段进入 active fact。
