...
| 条件 | 系统反应 |
|---|---|
| 高不确定性 + 高风险 | 降低行动，提升检索和来源验证，向用户澄清 |
| 高显著性 + 低风险 | 快速进入工作区并允许行动 |
| 高疲惫 + 低紧急度 | 推迟复杂输出，进入摘要/巩固/清理 |
| 高关系压力 | 降低推断强度，增加边界和确认 |
| 高奖励预期 + 高越界风险 | 由抑制层压制短期奖励 |
| 低边界完整性 | 隔离输入，不写长期记忆，不允许工具行动 |

## 边界声明

这些变量是功能调制，不是主观体验。`stress_pulse` 不是痛苦，`fatigue_load` 不是真实疲惫，`relationship_pressure` 不是孤独或依恋。它们的价值在于让数字生命有状态依赖的行为、恢复和成长，而不是把 AI 拟人化成生物体。

## 与 17/19/20 的连接

- `17_memory_trace_object_model.md` 在写入和检索时读取状态快照。
- `19_offline_consolidation_cycle.md` 根据疲惫、维护压力和 replay priority 触发离线周期。
- `20_agent_runtime_bridge_contract.md` 在工具行动前读取行动阈值、边界完整性和控制深度。
