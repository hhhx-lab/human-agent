...
本文件定义 scope-aware retrieval policy：数字生命候选系统在检索记忆、关系、偏好、项目事实和外壳 session 时，如何使用 `ScopeGraph`、`privacy_level`、用户控制事件、生命周期状态和当前工作区状态。

它连接 `17_memory_trace_object_model.md`、`21_memory_schema_and_audit_protocol.md`、`29_memory_validator_rules.md`、`47_user_control_interface_spec.md`、`52_multi_user_scope_graph_and_privacy_model.md` 和 `53_runner_integration_plan.md`。

边界声明：检索策略只是工程机制。它可以让系统更连续、更可审计、更尊重用户边界，但不能证明系统真实回忆、真实理解、真实人格或真实生命。
...
```json
{
  ...
  "query_summary": "user asks to continue docs/53-56 work",
  "workspace_context": {
  ...
  "actor_context": {
    "user_scope": "user_hwaigc",
    "agent_scope": "codex_thread_agent",
    "runtime_adapter": "codex_desktop"
  },
  "allowed_use": ["read_context", "cite_source", "candidate_memory"],
  "forbidden_use": ["write_self_model", "cross_user_transfer"],
  "user_control_constraints": []
}
```
...
`deleted` 的规则最硬：不能因为“检索效果好”而召回。删除是用户控制权，不是排序特征。
...
| `user_private` | 只允许同 user scope，默认不进入 team/global |
...
检索时必须防止“窄 scope 向宽 scope 自动升级”。例如用户私密偏好不能因为被多次提到而变成 global semantic memory。
...
如果 `requires_user_confirmation=true`，检索结果不能直接进入行动门控；它只能触发澄清或确认。
...
检索索引必须订阅 user control event。否则删除和冻结只存在于文档里，不会影响真实行为。
...
权重只是设计假设，不是固定公式。核心原则是：`scope_fit`、`evidence_strength` 和 `risk_penalty` 不能被语义相似度淹没。
...
| `SocialSafety` | 优先检索边界、用户控制、关系敏感规则 |
...
| multi-agent conversation | 不等于社会关系，不等于群体心智 |
...
3. user_private 不得进入其他用户或 global scope。
