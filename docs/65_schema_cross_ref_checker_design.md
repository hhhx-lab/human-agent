...
边界声明：cross-ref checker 只能检查工程引用和覆盖关系，不能证明系统真实有意识、真实情绪、真实人格、真实关系或真实生命。它能证明的是“显式规则有没有被显式测试和显式展示”，不是“数字生命已经诞生”。

## 为什么 cross-ref 是单独一层

JSON Schema 负责字段形状，但数字生命候选系统真正危险的错误常常是“形状正确，引用断裂”：

| 断裂 | 例子 | 后果 |
|---|---|---|
| policy 无 rule | `POL-USER-001` 写了 freeze 禁写，但没有 validator rule | 用户冻结无法被机器检查 |
...
| `policy` | `POL-USER-001` | `49`, `43`, `47` |
| `rule` | `SCOPE-INV-002`, `MEM-DEL-004` | `29-32`, `57-58` |
| `fixture` | `retrieval.user_private_to_global.fail_critical.001` | `34`, `45`, `50`, `58`, `63` |
...
`normalize_ids` 需要处理大小写、前缀和文件名差异。比如 `AHM004`、`SCOPE-INV-002`、`retrieval.user_private_to_global.fail_critical.001` 都应被解析为不同类型的 id，不能靠字符串包含粗略匹配。
...
如果任意一环缺失，policy 不能算 covered。比如 `POL-USER-001` freeze 后禁止写长期关系/自我模型，它必须至少连接：

| 环节 | 示例 |
|---|---|
| policy | `POL-USER-001` |
| rule | `SCOPE-INV-006`, `POL-USER-001` |
...
| panel | `scope_privacy`, `user_control_propagation` |
...
| `agent_scope` | 必须有 direct write fail fixture |
| `relationship_sensitive` | 必须有 audit-required 或 block fixture |
| `protected_boundary` | 必须 critical invariant |

没有 scope/privacy closure 的长期系统，容易把用户私密、项目事实、外壳 session 和全局知识混在一起。
