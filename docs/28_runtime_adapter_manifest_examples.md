...
本文件是 `24_runtime_adapter_test_suite.md` 的实例化层：为 LangGraph、OpenAI Agents SDK、Microsoft Agent Framework、Google ADK、Letta、LlamaIndex、CrewAI 和 AutoGen 写出 adapter manifest 样例。manifest 的目的，是把这些框架纳入可替换执行外壳和生命膜观测入口。

本文件引用已有矩阵锚点 [AHZ057-AHZ066, AHM013-AHM020]。框架接口会持续变化，因此真实实现必须在 adapter 发布前重新核对官方文档；生命层对象、写入门和审计协议不随外壳 API 漂移。

## 官方入口核对

以下入口用于定位当前外壳能力，并把外壳能力接到数字生命理论核心之外的感知、行动、追踪和回看通道。
...
LangGraph 的 checkpoint 能帮助回放行动路径。checkpoint 和 graph state 先作为轨迹证据进入生命膜，再由 `OfflineConsolidationCycle`、写入门和状态门生成长期生命材料。
...
ADK 的多外壳编排进入社会脑候选；它在执行层帮助拆分任务，随后由关系模型、共同记忆和责任链决定是否形成社会脑材料。
...
AutoGen 适合做多外壳交流入口；对话群体先形成群体观测候选，再由 `RelationshipModel`、共同记忆和关系后果生成群体心智材料。
