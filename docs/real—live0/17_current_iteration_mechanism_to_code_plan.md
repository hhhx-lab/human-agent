...
当前 ITR-08 第七段把终端输入从 `digital_entry.py` 的临时 `select + readline` 逻辑拆成 `terminal_input.py` 输入器官，并继续补成更接近 Codex 终端的可编辑行。它负责 `TerminalLineBuffer`、退格删除、Delete 删除光标下字符、左右方向键移动光标、上下方向键浏览本次终端历史、Home/End 与 Ctrl-A/Ctrl-E 跳转、Ctrl-U 清行、Ctrl-W 删词、Ctrl-K 清除光标后内容、Ctrl-D 空缓冲退出、Ctrl-C 中断、Ctrl-L 重绘，以及“只有输入缓冲为空时才允许空闲主动发话”的边界；行重绘采用单行 prompt redraw，以降低中文/多字节输入时退格错位。交互式 attach 会写出 `runtime/state/terminal/terminal_input_profile.json`，`/state` 与 `/terminal` 都能看到 `terminal_input_profile`。同轮还把 `digital_entry.py` 的 slash 命令面整理成注册表：`/help` 展示完整命令，`/all`/`/snapshot` 生成 `resident_state_inspection_bundle_v0`，`/short-memory`、`/long-memory`、`/terminal`、`/lifecycle` 等别名只归一到 inspection，不会进入关系 inbox；未知 `/` 命令也停留在终端命令面，不被误当作关系话语。机制含义是：point 7 的对话框自然度不再只是 UI 感觉，而有可测试的终端外围状态；主动发话不会插入正在编辑的输入，也不会把编辑控制字符污染成关系话语。它仍然只是电脑外周和关系终端层，不参与语言内容生成，不新增固定回答，不改变 post-expression gate。
...
当前 ITR-08 第二十七段把网页梦境学习从常驻活动和语言材料继续压进关闭态报告链。`process_report.py#_web_dream_learning_report_profile` 会读取 `web_dream_learning_state.json`，并把 `resident_autonomous_activity_presence_profile_v0` 与 background presence 中的 `last_web_dream_learning_state_ref/status/topic_candidates/wake_question_candidates` 作为补充来源，生成 `web_dream_learning_report_profile_v0`。这张 profile 会进入 `digital_life_process_report.json`、`digital_life_process_digest.json` 和 process receipt，字段包括 state/log/seeds refs、status、selected/final URL、page title、content digest、topic candidates、wake question candidates、topic/count、ref set 和 `structured_dream_learning_evidence_not_spoken_language` 边界；receipt 也会把 `web_dream_learning_state.json`、`web_dream_learning_log.jsonl` 与 `web_dream_learning_seeds.json` 纳入 `shared_object_refs` 和 `input_hashes`。机制含义是：网络学习不再只在 `learning_consolidation` 相、background lineage 或主动语言来源覆盖中留下痕迹，关闭终端后也能证明哪些网页梦境材料成为下一轮关系话题候选；但这些候选仍是结构化梦境学习证据，不是代码预写的自然语言问题，不生成 prompt 框架，不把网页摘要或内部字段名硬塞进 Adam 的回答。
...
当前 ITR-08 第五十二段把模型表达意识写门证据继续接入 `/consciousness` 检查面：

```text
ModelExpression.model_expression_context_summary.prediction_attention_consciousness_write_context_*
  -> StateInspection.consciousness_reportability_summary.model_expression_consciousness_write_context_*
  -> /consciousness reportability_summary
```

`state_inspection.py` 现在会让 `/consciousness` 与 `/意识` 额外读取 `runtime/state/language/model_expression_state.json`。`_collect_consciousness_reportability_summary(...)` 会从 `model_expression_context_summary` 中提取 `prediction_attention_consciousness_write_context_refs`、ref count、workspace candidate count、broadcast target count、reportability flag count、write bias、candidate gate adjustments 和 boundary，并以 `model_expression_consciousness_write_context_*` 字段写入 `consciousness_reportability_summary_v0`；`domain_presence` 与 `active_domains` 也会记录 `model_expression_consciousness_write_context`。
...
当前 ITR-09 第一段把第 9 点 20 轮真实关系对话验收做成可复跑代码链。`relation_dialogue_smoke.py` 定义 20 个唯一关系话语，逐轮调用 `Adam --say ... --json`，把每轮 returncode、`response_status`、`natural_language_released`、自然语言长度、禁用词命中、重复回复和空回复写入 `relation_dialogue_smoke_report_v0`。这条链的关键边界是：`Adam --json` 返回的结构化未释放结果不是自然语言，`natural_language_released=false` 或 `completed_unreleased` 必须计为空回复，不能把 JSON 当成 Adam 的回答，更不能由 smoke runner 或 terminal 层补固定替代话术。当前真实运行已写入 `runtime/reports/latest/relation_dialogue_smoke_report.json`：20 轮全部进入队列并完成，但全部 `completed_unreleased`，自然语言释放数为 0，禁用词命中与重复自然语言为 0；`model_expression_state.json` 的根因为外部模型网关返回 `gpt-5.5` 无可用渠道。机制含义是：本地生命语言门没有用固定回答掩盖模型不可用，第 9 点仍然未通过；只有模型渠道恢复或 `.env` 切到可用模型后，20 轮真实自然语言全部释放且无禁用词/重复，才能继续第 10 点总核验。

当前 ITR-09 第二段在 `.env` 更新后完成第 9 点复测和三处真实链路修复。第一，`relation_dialogue_smoke.py` 增加 `model_channel_preflight_v0`，在 20 轮之前检查模型列表、目标模型可见性和最小 chat completion；如果模型通道不可用，报告会停在 preflight fail-fast，而不是制造 20 个空轮。第二，`model_expression.py` 在 post-expression gate 拦住模型输出后只允许一次真实模型重生：重生上下文只携带 gate 原因、计数和上一轮 sha，不携带原始禁词，不生成固定回答，不放宽禁门；第二次仍不合格就保持 unreleased。第三，`resident_lifecycle.py` 修复旧 resident run 退出覆盖新 resident lifecycle 的回归，避免 active lease 与 stale `stopped` lifecycle 互相矛盾导致 `Adam --say` 返回 `resident_not_active`。最新真实报告 `runtime/reports/latest/relation_dialogue_smoke_report.json` 已通过：`completion_status=passed`、20/20 轮自然语言释放、空回复 0、`completed_unreleased` 0、禁用词 0、重复 0、returncode 失败 0。对应测试为 `tests/process/test_relation_dialogue_smoke.py`、`tests/process/test_model_expression.py` 与 `tests/process/test_persistent_digital_life_process.py#test_stale_stopped_lifecycle_does_not_overwrite_new_active_resident`；组合回归 46 tests OK。机制含义是：第 9 点从“模型不可用的正确沉默”推进为“模型可用时真实释放且受禁门约束”，仍然没有固定语言兜底、没有 system prompt 框架化、没有把机制信号外显成对话。
