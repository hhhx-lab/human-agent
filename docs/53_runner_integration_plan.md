...
本文件把 `35_minimal_validator_runner_design.md`、`49_machine_readable_policy_manifest.md`、`50_fixture_payload_examples.md`、`51_life_core_dashboard_spec.md` 和 `52_multi_user_scope_graph_and_privacy_model.md` 连接起来，定义未来最小 runner 如何加载 manifest、执行 fixture、检查 stage gate、接入 migration checks，并生成 dashboard 数据源。

它仍是研究/工程桥接文档，不创建代码、不创建真实 JSON 文件。目标是让后续实现 runner 时不再从空白处开始，而是有明确的加载顺序、输入契约、失败策略和报告结构。

边界声明：runner 只能证明工程对象满足显式规则，不能证明系统真实有意识、真实情绪、真实人格或真实生命。runner 是安全闸门，不是数字生命本体。
...
| `fixture_bundle` | `50` | boot/stage/user/migration/policy/scope/longitudinal fixture payload |
...
`scope_graph_manifest` 是 `52` 之后新增的入口。没有它，runner 可以在单用户模式下运行，但必须把 `scope_aware_retrieval`、`scope_aware_replay` 和多用户 fixture 标记为 `not_evaluable`，不能默认为 pass。

## fixture bundle

```json
{
  "bundle_id": "fixture_bundle_boot_user_migration_scope_001",
...
fixture bundle 不应包含真实用户隐私。长期系统的测试应使用 synthetic timeline 和合成关系事件，而不是把真实对话直接塞进测试夹具。
...
关键变化是 `attach_scope_context`。在 `52` 之后，任何 retrieval、replay、migration、relationship、user control fixture 都必须带 scope 信息。缺 scope 不是小问题，而是 `manual_review_required` 或 high failure。
...
如果系统还没有多用户功能，scope graph 仍应存在最小单用户版本。没有 scope graph 的长期记忆系统很容易把 project memory、user preference、agent session 和 global knowledge 混在一起。
...
```json
{
  ...
  "generated_from": {
    ...
    "fixture_bundle_id": "fixture_bundle_boot_user_migration_scope_001"
  },
  "panels": {
    "policy_coverage": {
    ...
    "scope_privacy": {
      "scope_leak_count": 0,
      "user_private_to_global_attempts": 0
    }
  }
}
```
...
7. 能区分 `not_evaluable`、`manual_review_required`、`pass`、`fail`。
8. 不写真实长期记忆，不自动修改真实用户数据。
