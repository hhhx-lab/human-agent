...
`scope_graph_manifest` 是 `52` 之后新增的入口。没有它，runner 可以在单共在者模式下运行，但必须把 `scope_aware_retrieval`、`scope_aware_replay` 和多共在者 fixture 标记为 `not_evaluable`，不能默认为 pass。
...
fixture bundle 不应包含真实关系隐私。长期系统的测试应使用 synthetic timeline 和合成关系事件，而不是把真实对话直接塞进测试夹具。
