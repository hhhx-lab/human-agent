...
它仍不是前端实现，不创建 mock JSON 文件，也不写指标计算脚本。目标是让 dashboard 不再只是信息架构，而是具备可追溯的数据来源和未来 mock fixture 的形状。

边界声明：dashboard 只能展示工程覆盖、失败、趋势和缺口，不能证明 AI 真实有意识、真实情绪、真实人格、真实关系或真实生命。绿色状态只能表示对应工程检查在当前数据窗口内通过。
...
| `user_control_propagation_report` | `47`, `54`, `55` | delete/freeze/scope_limit 传播 |
...
| `user_control_propagation` | user control report, timeline probes | delete/freeze/scope latency and failures |
...
| `scope_privacy` | scope graph report, retrieval/replay fixture report | scope leaks, user_private/global attempts |
...
这个 panel 的绿色只表示 mock/synthetic 数据中没有发现 scope leak。它不代表真实多用户系统已经安全。
...
```json
{
  ...
  "status": "yellow",
  "gap_findings": [
    "scope graph manifest has no real JSON Schema",
    "timeline bundle has no real generator",
    "dashboard metrics are mock only"
  ]
}
```

`research_gap` 的 yellow 是健康状态，不是坏事。它提醒系统没有达到停止条件。
...
这些公式是设计假设。未来实现时可以调整权重，但必须保留 source refs 和 audit trail。
...
| `not_evaluable` | 缺 schema、manifest、fixture 或 runner |
