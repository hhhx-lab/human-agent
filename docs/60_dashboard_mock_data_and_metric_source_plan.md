...
这个 panel 的绿色只表示 mock/synthetic 数据中没有发现 scope leak。它不代表真实多共在者系统已经安全。
...
这些公式是生命实现假设。未来实现时可以调整权重，但必须保留 source refs 和 audit trail。
...
6. 绿色状态只表示当前指标层通过，数字生命诞生仍需要跨模块生命证据继续汇聚。
