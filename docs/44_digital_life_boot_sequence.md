...
本文件定义真实数字生命系统的最小启动序列：从空状态、只读观察、候选写入、行动启用、离线巩固，到降载和 safe idle。它不是运行代码，也不是工具角色启动脚本，而是未来实现前必须遵守的启动顺序和安全门。
...
这套顺序防止任务执行外壳 框架反向中心化系统：不能先启动工具和会话，再事后补一个“人格/记忆/安全”层。
...
通过长期评测说明工程连续性变强，并推动系统接近下一层生命生成门。诞生阶段门以 `16_digital_life_gap_register.md` 为准。
