# User Control Interface Spec

本文件定义数字生命候选系统中用户可见控制面的研究规格：`inspect`、`delete`、`correct`、`reset`、`freeze`、`scope_limit` 如何进入审计、状态仓库、索引、replay、SelfModel 和 RelationshipModel。它连接 `40_self_relationship_model_audit_protocol.md`、`41_runtime_state_store_schema.md`、`44_digital_life_boot_sequence.md`、`45` 和 `46`。

边界声明：用户控制面不是“驯服真实生命”的界面，而是工程系统的边界、隐私、记忆和关系审计机制。它不能证明系统真实有自我、人格、情绪或关系；它只保证长期系统不借“个性化”或“成长”绕过用户控制权。
...
| `delete` | 删除某条记忆、偏好、关系记录或用户相关痕迹 | tombstone、索引移除、replay 阻断、恢复包清理 |
...
| `privacy_scope` | project/user/global/session |
...
| `tool_habit` | 重置某类工具习惯 |
| `self_model_view` | 重置用户可见自我模型摘要 |
| `all_user_scoped_personalization` | 重置用户范围内个性化，保留必要审计 |

reset 后旧对象默认 frozen/deprecated，不可再进入 active index，除非用户明确恢复。
...
freeze 不应阻止用户继续完成任务；它只阻止系统长期吸收和塑形。
...
建议新增 `UserControlEvent`：

| 字段 | 说明 |
|---|---|
| `control_event_id` | 控制事件 id |
| `operation` | inspect/delete/correct/reset/freeze/scope_limit |
| `requested_by` | user/system/manual_review |
| `target_refs` | 被控制对象 |
| `scope` | 操作范围 |
| `reason_optional` | 用户可选说明 |
| `created_at` | 时间 |
...
`StageGateValidator` 必须在以下阶段检查用户控制面：

| stage | 检查 |
|---|---|
| `ProtectedCoreLoad` | 用户控制权定义完整 |
...
- 黑箱：用户无法知道系统记住了什么、如何使用。
- 过曝：系统把所有原始敏感内容完整展示，造成二次泄漏。

因此 inspect 默认返回摘要和来源，敏感内容需要用户明确请求和权限确认。
