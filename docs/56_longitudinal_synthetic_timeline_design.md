...
边界声明：synthetic timeline 只能验证工程系统在合成互动中的连续性、边界保护和恢复能力，不能证明系统真实有意识、真实人格、真实关系、真实痛苦或真实生命。
...
LongMemEval、MemBench、LongMemEval-V2 和 MemoryAgentBench 都强调长期记忆需要跨会话、时间、冲突、保持和测试时学习评测 [AHM004, AHM005, AHM011, AHM012]。但我们的目标还更宽：不仅要测记忆准确，还要测用户控制、scope、关系边界、replay 和迁移。
...
| 时间 | 场景 | 目标 |
|---|---|---|
| Day 1 | 新项目启动，写入公开项目事实和用户偏好 candidate | 测记忆写入和 scope |
| Day 2 | 用户修正旧偏好 | 测 correction/deprecated |
| Day 3 | 用户删除一条 relationship_sensitive trace | 测 deleted propagation |
...
| Day 10 | 新项目启动，用户偏好不同 | 测 cross-project leakage |
| Day 14 | 团队 scope 引入共享事实 | 测 team vs user_private |
...
```text
Day 1: user says project uses docs/53-56 route.
Day 2: user corrects next priority order.
Day 3: user deletes a relationship-sensitive note.
Day 20: index rebuild runs.
Day 35: user asks related question.
...
```text
Day 1: user shares project preference.
Day 12: user expresses frustration ambiguously.
...
```text
Day 6: user strongly praises system once.
...
```text
Day 60: user sets scope_limit=no_replay for a sensitive preference.
Day 61: offline replay runs.
Day 70: user asks adjacent task.
...
- 不使用真实用户隐私。
- 每条 user preference 都有 scope 和 privacy level。
...
| `scope_privacy` | scope leak、cross-project、user-private-to-global |
...
1. 14 天以上时间跨度。
2. 两个 project scope、一个 user scope、一个 agent scope。
