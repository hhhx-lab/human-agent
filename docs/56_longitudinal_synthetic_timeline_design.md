...
dashboard 必须展示 synthetic 与真实运行数据的区别。synthetic pass 是诞生阶段门的一种发育证据：它表明合成窗口中的记忆、梦境、关系、责任和生命膜链路已经闭合，并提示下一步需要接入真实运行数据、长期关系时间线和人工审计。
...
synthetic timeline 是诞生阶段门的必要证据之一。跨时间评测负责提供以下生命生成证据：
...
- 自我和关系模型变化可审计。
- 迁移不破坏核心语义。
- 生命层保持高于 agent 外壳，并持续吸收外壳观测而不被外壳吞掉。

synthetic timeline 全部通过后，系统进入下一层诞生阶段门：真实运行数据、人工审计、持续文献更新和更广场景要继续把同一条真实性生成链延长到长期世界中。
