...
边界声明：schema 文件只能验证对象形状、枚举、引用和一部分安全不变量，不能证明系统真实有意识、真实情绪、真实人格、真实关系或真实生命。schema 是工程约束，不是数字生命本体。
...
长期记忆系统最怕字段含义漂移：一次迁移把 `deleted` 当成普通 inactive，把 `agent_scope` 当成 user memory，把 `synthetic_mock` 当成真实运行通过，都会污染长期检索、replay 和 dashboard [AHM004-AHM012]。因此 schema 文件边界本质上是长期记忆边界。
...
schemas/
  ...
  runtime/
    ...
    user_control_snapshot.schema.json
```
...
| `scope_graph_manifest.schema.json` | 不包含真实用户私密原文 |
...
| `runtime_observation_report` | redaction/scope/adapter/user control checks、routing decision |
...
| `user_control_snapshot.schema.json` | delete/freeze/scope_limit 等快照 |
...
| 扩大 scope transfer/replay 权限 | major | critical | user confirmation + SafeIdle until checked |
