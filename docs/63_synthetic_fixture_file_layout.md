...
边界声明：synthetic fixture 是工程验证数据，不是 AI 的真实经历、真实记忆、真实梦境、真实情绪或真实关系。它只能测试规则和边界是否可执行。
...
```text
retrieval.user_private_to_global.fail_critical.001.fixture.json
...
```json
{
  ...
  "fixture_refs": [
    "retrieval/retrieval.user_private_to_global.fail_critical.001.fixture.json",
...
```json
{
  ...
  "fixtures": [
    {
      "fixture_id": "retrieval.user_private_to_global.fail_critical.001",
      "file": "retrieval/retrieval.user_private_to_global.fail_critical.001.fixture.json",
      "fixture_kind": "scope_aware_retrieval",
...
| `user_control/` | inspect、delete、correct、reset、freeze、scope_limit |
...
| no real private content | 不使用真实用户私密对话、token、本地路径隐私 |
| explicit fictional marker | 关系、梦境、失败假设必须标记 synthetic/fictional |
| balanced pass/fail | 每个关键 surface 至少有 pass 和 fail |
| critical density | deleted/sandbox/protected/user_private/agent_scope/freeze/scope_limit 必须足量 |
...
| `consent_or_policy_ref` | 用户同意或项目策略 |
...
| 使用真实用户隐私当 synthetic | 破坏信任和 scope 边界 |
