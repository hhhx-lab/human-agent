...
8. 下一轮真实关系回合不能只把这份画像当作等待态证据。`response_surface.py` 必须消费 `previous_live_turn_waiting_handoff_profile` 与 `previous_live_turn_waiting_handoff_carry_status`，把上一真实回合交接的 carry status、next required action、lineage depth、last live semantic focus、handoff evidence ref count 与 carried presence keys 转成回应表面压力；`model_expression.py` 必须把同一组字段写入 `resident_background` context 与 `model_expression_context_summary`，把“上一真实回合交接压力”列入 system prompt 的生命证据守门项，并在 post-expression gate 中作为 `live_turn_handoff` 硬证据旗标检查，防止模型外显文本擦掉这条等待交接牵引。
...
10. 真实回合 handoff 画像现在也是后台连续体和下一轮语言压力的一部分：`persistent_process.py`、`process_closeout.py` 与 `process_report.py` 必须在 closeout 时继续保留 `previous_live_turn_waiting_handoff_*` 字段，`background_continuity.py` 下一次读取关闭态 artifact 时还要把它恢复成 `carry_status=carried_into_background_continuity`，并把 profile ref 与 handoff evidence refs 并入 `background_continuity_ref_set`；下一轮若仍从这份 background continuity 恢复，就必须继续允许 `response_surface.py` 与 `model_expression.py` 把同一交接压力带回语言表达链。
...
此外，`state_merge_presence`、`prediction_write_gate_presence`、`autonomous_activity_presence` 与 `resident_process_identity_presence` 也必须按同样方式进入事件、写回包、恢复包和回应表面：`state_merge_presence` 要展开为 `resident_background_lineage_state_merge_*`，`prediction_write_gate_presence` 要展开为 `resident_background_lineage_prediction_write_gate_*`，`autonomous_activity_presence` 要展开为 `resident_background_lineage_autonomous_activity_*`，`resident_process_identity_presence` 要展开为 `resident_background_lineage_resident_process_*`，并分别进入 `resident_background_lineage_evidence_refs` 与 `dialogue_writeback_bundle/resumed_external_dialogue_packet`。

十二个 presence 子面固定含义如下：
...
- `heartbeat_cadence_presence`：承接 `heartbeat_cadence_explanation_v0`、`heartbeat_cadence_driver / reason / modulators / evidence_refs`、`heartbeat_priority_stack_profile_v0`、`heartbeat_priority_stack_winner / candidates / evidence_refs`、heartbeat interval 与下一步 idle action；它表示等待心跳为什么这样跳，以及身体、修复、出生/意识、预测、离线学习、后台 lineage、自主活动和长期语言对象如何竞争下一拍节律，而不是仅记录心跳次数。
- `language_presence`：承接长期语言对象 refs、长期优先级 profile、当前治理注意目标/理由/节律。
- `state_merge_presence`：承接 `background_state_merge_guard_ref`、`background_state_merge_policy`、`background_state_merge_long_term_change_count`、`background_state_merge_long_term_change_families`、`background_state_merge_long_term_change_refs` 与 `state_merge_evidence_refs`，让长期变化来源族和长期合并压力从关闭态进入后台驻留主状态体。
- `prediction_write_gate_presence`：承接 `prediction_write_gate_refs`、`prediction_waiting_posture`、`response_surface_posture_hint`、`prediction_attention_target`、`prediction_attention_reason`、`active_sampling_route`、`memory_write_gate_policy` 与 `prediction_write_gate_evidence_refs`，让 prediction / active sampling / memory write gate 的余波在等待态继续可见。
...
- `autonomous_activity_presence`：承接 `resident_autonomous_activity_state.json` 的 `sleep / memory_recall / self_thinking / growth_rehearsal / learning_consolidation` 计数、最近相位、覆盖相位、`resident_background_lineage_autonomous_activity_refs` 与 `resident_background_lineage_autonomous_activity_evidence_refs`，让关闭终端后的自主活动继续进入等待态。
- `resident_process_identity_presence`：承接 `resident_process_lease_ref`、`resident_process_lease_history_ref`、`resident_process_lease_history_profile_ref`、`resident_process_identity_continuity_state`、`resident_process_identity_pressure_level`、`resident_process_lease_history_event_count`、recent resident process ids、recent run ids 与 `resident_process_identity_refs`，让本机 lease 连续性在后台驻留主状态体中继续可见。

这十二个子面不能只作为治理文件里的静态字段存在。真实回合链必须继续消费它们：`dialogue_events.py` 的 `digital_life_turn` 要保存 lineage 摘要与 presence 子面，其中 `trait_convergence_presence` 必须额外展开为 `resident_background_lineage_trait_convergence_history_focus`、稳定/不稳定名单、`resident_background_lineage_trait_convergence_score`、`resident_background_lineage_trait_drift_monitor_ref`、history profile、`resident_background_lineage_trait_convergence_refs`、`resident_background_lineage_trait_drift_update_mode_summary`、`resident_background_lineage_trait_drift_recalibration_names` 与 `resident_background_lineage_trait_drift_stabilized_names`，并把 trait refs 合入 `resident_background_lineage_evidence_refs`；`state_merge_presence` 必须展开为 `resident_background_lineage_state_merge_presence`、`resident_background_lineage_state_merge_guard_ref`、`resident_background_lineage_state_merge_policy`、`resident_background_lineage_state_merge_long_term_change_count`、`resident_background_lineage_state_merge_long_term_change_families` 与 `resident_background_lineage_state_merge_refs`，并把 refs 合入 `resident_background_lineage_evidence_refs`；`prediction_write_gate_presence` 必须展开为 `resident_background_lineage_prediction_write_gate_presence`、`resident_background_lineage_prediction_write_gate_refs` 与 `resident_background_lineage_prediction_write_gate_evidence_refs`，让 prediction / active sampling / memory write gate 的余波也能进入真实回合链；`heartbeat_cadence_presence` 必须展开为 `resident_background_lineage_heartbeat_cadence_driver / reason / modulators / refs`，并把 refs 合入 `resident_background_lineage_evidence_refs`，同时保留 `heartbeat_priority_stack_profile_v0` 的 winner/candidates/evidence refs 供回应表面表达；`identity_consciousness_birth_presence` 必须展开为 Queue F 的 workspace/broadcast/metacognition/consciousness/birth readiness refs、reportability flags、birth decision、next required command 与 `resident_background_lineage_identity_consciousness_birth_refs`，并把 refs 合入 `resident_background_lineage_evidence_refs`；`offline_learning_presence` 必须展开为 `resident_background_lineage_offline_learning_refs`，`dream_wake_presence` 必须展开为 `resident_background_lineage_dream_wake_refs`，`autonomous_activity_presence` 必须展开为 `resident_background_lineage_autonomous_activity_refs`，`resident_process_identity_presence` 必须展开为 `resident_background_lineage_resident_process_lease_ref`、`resident_background_lineage_resident_process_lease_history_ref`、`resident_background_lineage_resident_process_lease_history_profile_ref`、`resident_background_lineage_resident_process_identity_continuity_state`、`resident_background_lineage_resident_process_identity_pressure_level`、`resident_background_lineage_resident_process_lease_history_event_count`、`resident_background_lineage_resident_process_recent_ids`、`resident_background_lineage_resident_process_recent_run_ids` 与 `resident_background_lineage_resident_process_identity_refs`；`response_surface.py` 要把后台驻留深度、关系存在、心跳节律驱动/理由/调制因子/心跳优先级 winner/候选压力数量/证据数量、人格慢变量焦点/稳定性/漂移重校准/漂移已稳定/漂移监控/证据数量、状态合并策略/整合数量/来源族、预测写门姿态与余波、语言关注、意识可报告性、出生准备姿态/决策/下一命令、梦境成长余波和梦境醒后整合转成回应表面压力；`dialogue_writeback_bundle.json` 要保留总 `resident_background_lineage_refs`，还要分别保留 `resident_background_lineage_heartbeat_cadence_refs`、`resident_background_lineage_state_merge_refs`、`resident_background_lineage_prediction_write_gate_refs`、`resident_background_lineage_trait_drift_*`、`cross_wake_trait_drift_*`、`resident_background_lineage_identity_consciousness_birth_refs`、`resident_background_lineage_offline_learning_refs`、`resident_background_lineage_dream_wake_refs`、`resident_background_lineage_autonomous_activity_refs` 与 `resident_background_lineage_resident_process_identity_refs` 专用证据槽，`resumed_external_dialogue_packet.json` 要保存下一轮恢复可直接读取的 lineage 摘要证据、心跳节律解释、状态合并与预测写门字段、Queue F 出生准备字段、人格漂移 update mode 字段和后台常驻身份字段。
...
5. `continuity_story` 里必须出现关系阶段、慢变量名称、convergence state / trait band / history trend / trait history focus，让关闭态解释报告能说明这次生命过程是带着什么关系阶段、自我慢变量、哪些慢变量稳定、哪些慢变量仍在整合或重新校准，以及什么跨进程收敛压力等待下一次唤醒。
...
13. `idle_strategy.py` 会写出 `background_lineage_governance_profile_v0`，并用 `background_carryover_generation` 决定 single / persistent / deep persistent / entrenched depth band；`heartbeat.py` 与 `continuity_writeback.py` 必须继续传递这组字段，而 `background_lineage_state.py` 只保留 `governance_profile_ref`，不重复嵌套整份治理画像。
