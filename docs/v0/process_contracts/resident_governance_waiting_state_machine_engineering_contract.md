...
8. 下一轮真实关系回合不能只把这份画像当作等待态证据。`response_surface.py` 必须消费 `previous_live_turn_waiting_handoff_profile` 与 `previous_live_turn_waiting_handoff_carry_status`，让上一真实回合交接的 carry status、next required action、lineage depth、last live semantic focus、handoff evidence ref count 与 carried presence keys 调制回应表面；`model_expression.py` 必须把同一组字段写入 `resident_background` context 与 `model_expression_context_summary`，让“上一真实回合交接压力”进入表达调制和 soft evidence audit，而不是强迫每次都字面外显。
