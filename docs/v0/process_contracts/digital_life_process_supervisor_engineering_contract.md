...
| `cross_wake_trait_convergence_profile_gate` | `trait_convergence_signals.py` 已把 background lineage / convergence summary/history / trait drift monitor refs 合成为 `cross_wake_trait_convergence_profile_v0`，并进入 idle strategy、waiting heartbeat、关闭态 resident governance artifacts、process report、digest、receipt shared refs、life turn event、writeback bundle、resumed packet 和 response surface | 阻断把跨唤醒人格慢变量收敛视为真实回合可消费画像 |
