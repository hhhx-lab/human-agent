...
20. `background_convergence.py` 必须把跨进程恢复出的关系阶段、自我慢变量和当前 bootstrap 后状态压成 `background_convergence_summary.json`；`idle_strategy.py` 必须让 `background_convergence_pressure_level` 调制 resident governance attention/cadence，`process_report.py` 与 receipt 必须把这份 summary 作为一级 shared object 与 input hash 保留。
