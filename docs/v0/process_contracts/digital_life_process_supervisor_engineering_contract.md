...
20. `background_convergence.py` 必须把跨进程恢复出的关系阶段、自我慢变量和当前 bootstrap 后状态压成 `background_convergence_summary.json`；`background_convergence_history.py` 必须把最近多次唤醒的 convergence state / pressure / trait score 压成 `background_convergence_history.json`；`idle_strategy.py` 必须让 `background_convergence_pressure_level` 与 history trend 调制 resident governance attention/cadence，`governance_explanation.py` 必须把它解释成 `background_trait_convergence_hold` 或 `background_convergence_recalibration`，`process_report.py` 与 receipt 必须把 summary/history 作为一级 shared object 与 input hash 保留。
