...
20. `background_convergence.py` 必须把跨进程恢复出的关系阶段、自我慢变量和当前 bootstrap 后状态压成 `background_convergence_summary.json`；`idle_strategy.py` 必须让 `background_convergence_pressure_level` 调制 resident governance attention/cadence，`governance_explanation.py` 必须把它解释成 `background_trait_convergence_hold` 或 `background_convergence_recalibration`，`process_report.py` 与 receipt 必须把这份 summary 作为一级 shared object 与 input hash 保留。
...
8. closeout 后必须额外写出 `digital_life_resident_governance_explanation.json`，把当前 cadence、governance driver、background continuity lineage depth、background relationship stage、trait slow variables、background convergence focus 与下一次唤醒预期显式解释出来，而不是只把这些信息埋在 state/report 字段里。
