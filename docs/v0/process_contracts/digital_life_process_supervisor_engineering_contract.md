...
14. 每个带有 `resident_background_lineage_state.body_presence` 的下一轮真实回合，还必须证明身体内环境 presence 进入事件、专用写回槽、恢复包和回应：`digital_life_turn` 带 `resident_background_lineage_body_waiting_posture`、疲惫、睡眠压力、能量、修复驱力、核心唤醒、痛苦压力、责任重量与 `resident_background_lineage_body_refs`；`dialogue_writeback_bundle.resident_background_lineage_body_refs` 和总 `resident_background_lineage_refs` 保留同一组身体证据 refs；`resumed_external_dialogue_packet.json` 继续保留这些 refs；回应文本表达后台身体等待姿态、疲惫负载、睡眠压力、能量状态、身体修复驱力、核心唤醒度和证据数量。
