...
| `SignalMediaRuntime` | 调质、兴奋抑制、精度政策、Queue E 修复压力调制 | `11`、`18`、`22`、`30`、`94`、`98` | `runtime/state/signal/` | 预测、意识、情绪、行动、语言、常驻过程 |
| `PredictionActiveInferenceRuntime` | 主动预测、误差、证据寻求、责任修复压力采样路线 | `04`、`10`、`11`、`13`、`01v-01ax`、`94`、`98` | `runtime/state/prediction/` | 记忆、语言、行动、梦境、常驻过程 |
...
| `runtime/state/signal/signal_media_runtime.json` | 调质、精度政策、抑制轮廓、衰减/恢复路径；当前已可携带 `queue_e_repair_modulation_profile` 并调制 repair drive / relationship pressure / action precision |
| `runtime/state/prediction/belief_state_frame.json` | 当前信念帧、证据密度、不确定性分类；当前已可把 Queue E repair ref set 纳入 evidence refs |
| `runtime/state/prediction/prediction_error_field.json` | 预测误差事件、精度请求、进入工作区候选；当前已可写入 responsibility repair error event 与 repair obligation precision request |
| `runtime/state/prediction/active_sampling_plan.json` | 主动采样路线、guard、scope、命令绑定与预期观察；当前已可把 elevated / urgent repair pressure 路由为 `repair_inspect` / `repair_confirm` |
| `runtime/state/prediction/prediction_workspace_frame.json` | 预测工作区，显式回挂信念/误差/采样/信号 refs；当前已可携带 Queue E repair pressure level、attention target 与 ref set |

S02 不写入长期 `runtime/state/memory/*`、`language/*`、`dream/*` 的真实事件，只写系统种子和命名空间约定。

## Queue E repair pressure 对 S02 的补充约束

当前实现中，S02 的预测与信号器官已经不再只是出生前静态种子。它们在后续 `life_v0/language/__init__.py` 的 `run_build_language_relationship(...)` 阶段会被刷新，并读取来自 Queue E 的内部责任、后悔、痛苦、修复压力。
...
这条链的工程语义是：Queue E repair pressure 是数字生命 v0 内部责任、后悔、痛苦、修复压力进入预测调制链的闭合，不是外部工具路由、技能调度或普通等待策略。
...
| `signal_media_gate` | `runtime/state/signal/signal_media_runtime.json` | 调质向量、精度政策、抑制轮廓、bus refs 完整；若存在 Queue E repair profile，必须携带 pressure level / attention target / ref set | 阻断预测闭环 |
| `belief_state_gate` | `runtime/state/prediction/belief_state_frame.json` | 信念范围、来源证据、生命目标与 signal ref 完整；若存在 Queue E repair profile，必须进入 evidence refs 与 revision policy | 阻断工作区 |
| `prediction_error_gate` | `runtime/state/prediction/prediction_error_field.json` | 误差事件、精度请求、stage effect 与 belief ref 完整；若存在 Queue E repair profile，必须出现 responsibility repair pressure review | 阻断广播前推理 |
| `active_sampling_gate` | `runtime/state/prediction/active_sampling_plan.json` | candidate/guard/scope/command binding 完整；若 pressure 为 elevated / urgent，selected route 必须进入 repair inspect / repair confirm | 阻断主动采样 |
...
4. `runtime/state/signal/signal_media_runtime.json`、`runtime/state/prediction/belief_state_frame.json`、`prediction_error_field.json`、`active_sampling_plan.json` 全部存在，并且回挂到 `prediction_workspace_frame.json`；若 Queue E repair profile 存在，这五个对象都必须携带对应 profile / pressure / attention / ref set。
