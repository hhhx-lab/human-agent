...
| `runtime/state/signal/signal_media_runtime.json` | language build 阶段刷新后的信号介质对象，携带 Queue E repair modulation profile |
| `runtime/state/prediction/belief_state_frame.json` | language build 阶段刷新后的信念帧，携带 Queue E repair evidence refs |
| `runtime/state/prediction/prediction_error_field.json` | language build 阶段刷新后的预测误差场，携带责任修复压力误差事件 |
| `runtime/state/prediction/active_sampling_plan.json` | language build 阶段刷新后的主动采样计划，repair pressure elevated / urgent 时进入 `repair_inspect` / `repair_confirm` |
| `runtime/state/prediction/prediction_workspace_frame.json` | language build 阶段刷新后的预测工作区，回挂 signal / belief / error / sampling 与 Queue E repair profile |
...
当前 `life_v0/language/__init__.py` 还承担一条重要的跨层刷新职责：在 `run_build_language_relationship(...)` 中读取 `responsibility_loop_state`、`world_contact_summary`、`pain_regret_repair_report`，调用 `life_v0/membrane/queue_e_signals.py` 生成 `queue_e_repair_modulation_profile_v0`，再刷新 `signal_media.py`、`belief_state.py`、`prediction_error.py`、`active_sampling.py` 与 `prediction_workspace.py`。这表示 S07 不只消费预测工作区，也会在语言阶段把关系责任压力反写回预测链。
...
本轮还必须吸收同一组 Queue E repair modulation 字段：
...
这些字段来自 `runtime/state/action/responsibility_loop_state.json`、`runtime/state/membrane/world_contact_summary.json` 与 `runtime/reports/latest/pain_regret_repair_report.json`，并会被写回 signal / prediction runtime state。S07 不得把它们降格为一次性语言修辞或普通等待理由。
...
`build-language-relationship` 的当前真实输入除了 `--membrane runtime/state/membrane` 外，还会从 `runtime/state` 与 `runtime/reports/latest` 读取：
...
| `queue_e_prediction_refresh_gate` | language build 阶段能把 Queue E repair modulation profile 写入 signal / belief / prediction error / active sampling / prediction workspace | 阻断 prediction continuity |
...
| `QueueERepairModulationProfile` | `membrane/queue_e_signals.py`，由 `language/__init__.py` 调用 | `signal_media.py`、`belief_state.py`、`prediction_error.py`、`active_sampling.py`、`prediction_workspace.py`、`process_supervisor` |
...
6. `build-language-relationship` 至少能把 Queue E repair modulation profile 写入 `signal_media_runtime.json`、`belief_state_frame.json`、`prediction_error_field.json`、`active_sampling_plan.json` 与 `prediction_workspace_frame.json`。
...
- 当前 Queue E repair pressure 是否已经把下一轮预测路线推向修复审查或修复确认。
