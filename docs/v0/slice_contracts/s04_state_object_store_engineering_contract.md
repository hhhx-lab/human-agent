...
| `memory/memory_validator_report.json` | MemoryTraceValidator 的 claim 分区、retrieval/replay 阻断、防伪守门、夹具结果和下游消费者 refs |
...
| `memory_validator_gate` | `runtime/state/memory/memory_validator_report.json` | fact/hypothesis/dream/counterfactual/relationship inference 分区齐全，deleted/quarantined/sandboxed 阻断检索和 replay，梦境/沙盒/关系读心/修正覆盖防伪规则存在 | 阻断长期记忆召回 |
...
7. `memory/memory_validator_report.json` 存在，并固定 `claim_partition_index`、`retrieval_replay_guard`、`falsification_guard`、`falsification_fixture_results`，且 `life_state.memory_index.memory_validator_refs`、`MemoryRetrievalFrame.blocked_or_quarantined_refs`、`MemoryWriteGate.long_term_governance_refs` 和 `StateMergeGuard.long_term_change_sources` 都能回链它。
8. `memory/state_merge_guard.json` 存在，并固定 promotion、quarantine、repair、merge 四组长期治理 route。
9. `relationship_memory.json` 标出 `prediction_error_resolution_refs` 与 `offline_learning_writeback_refs`，`life_state.json` 标出 `state_merge_records` 与 `memory_index.state_merge_guard_refs`。
10. `lifecycle_policy.json` 固定 deleted、quarantined、sandboxed、protected、frozen 的检索、replay、写入和迁移语义。
11. `state_store_report.json` 下一步只能指向 `S03_DIRECTION_LIFE_MEMBRANE`。
12. `life-v0 check-state-store --strict` 返回 `status=closed`。
