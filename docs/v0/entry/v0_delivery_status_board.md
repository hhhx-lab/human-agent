...
最新 ITR-05 补强：身体/情绪信号已经从第一段预测-记忆写门链推进到第二段跨唤醒写回链。`signal_media.py` 从 `body_resource_budget.json` 和 `core_affect_vector.json` 合成 `body_signal_profile`，`memory_write_gate.py` 生成 `body_signal_write_modulation`，`language/__init__.py` 在实时语言阶段刷新 signal 后同步投射并重写 `memory_write_gate.json`；随后 `live_turn_cycle.py` 会在真实回合开头用当前身体/核心情绪/调质再投射一次写门，`background_continuity.py` 会从上一轮 governance、snapshot、process report 和 nested `prediction_write_gate_presence` 恢复 `background_body_signal_*`，`idle_strategy.py` 放行这组字段，`resident_turn_writeback.py` 与 `dialogue_writeback.py` 把 `resident_background_lineage_body_signal_*` 写入写回包和恢复包，`process_report.py` 则把 `body_signal_ref_set`、write bias 与 candidate gate adjustments 写入 report、digest 和 receipt。状态板因此把这一格视为“身体内环境已进入调质、写门、驻留谱系、回应审计、写回包、恢复包和关闭态归档的第二段闭环”，仍不是 ITR-05 或第 2 点整体完成；下一步转向更宽测试验证和梦境/offline pressure 的长期累计深消费，而不是重复检查已接通的 writeback/background/report 字段。
