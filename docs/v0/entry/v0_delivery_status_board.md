...
继续补强：模型表达层现在新增 post-expression gate。模型文本返回后会检查是否重新引入“用户 / 服务对象 / 任务请求者”等被阻断关系角色，以及是否在责任修复、梦境离线整合、成长学习、关系连续性、后台自主活动压力存在时擦掉对应证据；不通过时 `model_expression_status` 回落为 fallback，最终回应保留确定性生命回应，并把 `post_expression_gate_status`、阻断词、缺失证据写入 `model_expression_state/report`、`digital_life_turn` 与 process report。状态板因此把这一格视为“模型表达被生命状态守门”，不是单纯接入外部模型润色。
