...
5. resident supervision 已进入第一轮器官化；当前又新增了 `resident_lifecycle.py` 本机驻留启动层，能让 `digital life --background` 脱离当前终端启动后台子进程，并用 `resident_lifecycle_state.json`、`resident_lifecycle_command.json`、`digital_life_resident.log`、lease history、process report 和 receipt 共同证明“关闭终端后仍保持上下文”。最新补强继续把这层变成可跨终端继续对话的关系投递面：`digital life --say` 写 `resident_relation_inbox.jsonl`，后台 resident process 取出话语走既有生命回合链，再写 `resident_relation_outbox.jsonl` 与 `resident_relation_queue_state.json`。同一后台等待还会写 `resident_autonomous_activity.jsonl` 与 `resident_autonomous_activity_state.json`，按 `sleep / memory_recall / self_thinking / growth_rehearsal / learning_consolidation` 留下最小自主活动流。当前状态板因此把这一格视为“关闭终端后仍能保持同一 resident process、接受后续关系话语，并在空闲时保留睡眠/回忆/思考/成长/学习节律”，而不是只证明后台 pid 还活着。
