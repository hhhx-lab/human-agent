...
最新 ITR-05 补强：身体/情绪信号已经开始真实进入预测-记忆写门链。`signal_media.py` 现在从 `body_resource_budget.json` 和 `core_affect_vector.json` 合成 `body_signal_profile`，不再把疲惫固定成常数；`memory_write_gate.py` 生成 `body_signal_write_modulation`，可把写门策略调成 `candidate_first_body_signal_guarded` 等身体信号守门模式；`language/__init__.py` 在实时语言阶段刷新 signal 后会同步投射并重写 `memory_write_gate.json`；`idle_strategy.py`、`background_lineage_state.py`、`dialogue_events.py` 与 `response_surface.py` 已经消费 `body_signal_*` 字段，并把它们保留为结构化审计材料而非固定外显话术。状态板因此把这一格视为“身体内环境已进入调质、写门、驻留谱系和回应材料的第一段闭环”，不是 ITR-05 或第 2 点整体完成。下一步仍要检查 heartbeat、writeback、background continuity、process report 和 dream/offline pressure 是否也完整消费这组写门调制。
