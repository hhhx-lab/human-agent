...
4. 项目级 packaging / installable command surface 已接通；下一步全局长期运行层先从 `runtime/state/terminal/resident_process_lease.json` 与 `runtime/state/terminal/resident_process_lease_history.jsonl` 这种常驻生命进程身份对象开始，而不是直接跳到 daemon 或工具网关。该 lease 必须把 active waiting、heartbeat refresh、`resident_process_id`、lifecycle history、active lease relaunch normalization、closeout、persistent process artifacts、process report/digest/receipt 接成同一条可审计进程身份链。
