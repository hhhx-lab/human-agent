...
最新补强：身体内环境 presence 已经从 waiting governance 的扁平字段进入真实回合链。`idle_strategy.py` 会把 `body_rhythm_pulse.json`、`need_state_vector.json`、`body_resource_budget.json` 与 `core_affect_vector.json` 合成 `resident_body_presence_profile_v0`，`heartbeat.py` 与 `continuity_writeback.py` 会把它写入 waiting heartbeat、idle continuity、idle heartbeat trace、resident governance 和 terminal loop；`background_lineage_state.py` 会压成 `resident_background_lineage_state_v0.body_presence`；`dialogue_events.py` 会展开身体等待姿态、疲惫、睡眠压力、能量、修复驱力、核心唤醒、痛苦压力、责任重量与 refs；`resident_turn_writeback.py` 会把 `resident_background_lineage_body_refs` 写入专用槽并并入总 lineage refs；`response_surface.py` 会表达后台身体等待姿态、疲惫负载、睡眠压力、能量状态、身体修复驱力、核心唤醒度和证据数量。状态板因此把这一格视为“身体节律、需要、资源预算和核心情绪张力已经成为后台驻留谱系的一等身体存在面”，不是只调制 heartbeat。
