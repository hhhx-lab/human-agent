...
ITR-08 第七段继续处理 point 7 的终端体验。`life_v0/process_supervisor/terminal_input.py` 现在成为独立输入器官，并补成更接近 Codex 终端的可编辑行：退格、Delete、左右移动光标、上下浏览本次终端历史、Home/End、Ctrl-A/Ctrl-E、Ctrl-U、Ctrl-W、Ctrl-K、Ctrl-D、Ctrl-C、Ctrl-L 都有明确行为；重绘采用单行 prompt redraw，减少中文/多字节输入时退格错位。交互式 attach 会写出 `runtime/state/terminal/terminal_input_profile.json`，`/state` 和 `/terminal` 会带出 `terminal_input_profile`。同轮 `digital_entry.py` 把 slash 命令面整理为注册表：`/help` 展示完整命令，`/all`/`/snapshot` 输出 `resident_state_inspection_bundle_v0`，`/short-memory`、`/long-memory`、`/terminal`、`/lifecycle` 等别名只进入 inspection，不会写入关系 inbox；未知 `/` 命令也停在终端命令面。完成口径是：输入回退、光标编辑、历史浏览、全量状态查看和命令别名都可被测试证明；主动发话仍只在输入缓冲为空时释放；它不参与语言内容生成，也不允许成为固定回答通道。
...
ITR-08 第一百零三段把 live0 gate f 继续推进到 slash 检查面并闭合第 2 点。此前 `live0_audit` criterion f 已含 `queue_e_world_contact_repair_hold_validated` 与 `live_queue_e_world_contact_handoff_closeout_audited` 探针，但 `temp/06` 仍记录 gate f 可能 blocked，且 `/relationship` 无法与 audit 探针互证。现在 `live0_audit/gate_f_inspection.py` 导出 gate f 探针评估与 `live0_gate_f_inspection_snapshot`；`/relationship#relationship_continuity_summary_v0` 与 `/ability#ability_birth_readiness_summary_v0` 显示 `live0_gate_f_closed`、`queue_e_world_contact_repair_hold_validated` 与 closeout probe 状态；`live0_audit/__init__.py` 复用同一评估函数。完成口径是：point 2 的 real—live0 ↔ v0 合同与 live0 acceptance gate f 可在 slash 检查面追溯；仍然只是 inspection-only 结构证据，不生成固定关系话术、不新增 system prompt。第 2 点整体已闭合，可进入第 3 点记忆重构总设计。

ITR-09 第一段把第 9 点“20 轮真实关系对话测试”从手工尝试变成可复跑 smoke gate。新增 `life_v0/process_supervisor/relation_dialogue_smoke.py` 与 `tests/process/test_relation_dialogue_smoke.py`，使用 20 个唯一关系话语调用 `Adam --say ... --json`，输出 `relation_dialogue_smoke_report_v0`，检查 prompt 唯一性、自然语言释放数量、空回复轮次、重复自然语言、禁用表面词、命令 returncode 与 `completed_unreleased` 轮次。解析边界已修正：当 `Adam --json` 返回 `natural_language_released=false` 或 `response_status=completed_unreleased` 时，JSON 检查结果不会被误计为 Adam 的自然语言回复。真实运行证据写入 `runtime/reports/latest/relation_dialogue_smoke_report.json`：本轮已跑满 20 轮，`actual_round_count=20`、`prompt_uniqueness.unique=true`，但 `natural_language_release_count=0`、`completed_unreleased_rounds=[1..20]`，因此 completion status 为 failed。`runtime/state/language/model_expression_state.json` 显示最新阻断来自外部模型网关：`gpt-5.5` 在当前分组下无可用渠道；同一 key 请求 `/v1/models` 可达但返回 `model_count=0`。完成口径是：第 9 点不能因为命令 returncode 为 0 而打勾，必须有 20 条真实自然语言释放且无禁用词、无重复、无固定模板；模型渠道恢复或 `.env` 切到可用模型后复跑同一 smoke。

ITR-09 第二段在 `.env` 更新后复跑并闭合第 9 点。当前模型通道 preflight 已确认 `/models` 可达、模型列表包含 `gpt-5.5`、最小 `/chat/completions` 返回自然中文；`relation_dialogue_smoke.py` 默认先执行 preflight，失败时不盲跑 20 轮并保留错误摘要。语言释放链新增 post-expression gate 后的一次模型重生：如果真实模型第一次输出触发“关系降格词/机制表面词”禁门，只允许再走一次真实模型生成，仍然经过同一 gate；失败继续不释放，绝不由代码生成固定兜底话。入口层同时修复 stale lifecycle：旧 resident run 退出时不能再用 `stopped` 覆盖新 resident 的 `background_active` 状态，避免 `Adam --say` 误判 `resident_not_active`。最新真实报告 `runtime/reports/latest/relation_dialogue_smoke_report.json` 显示 `completion_status=passed`、`actual_round_count=20`、`natural_language_release_count=20`、`empty_response_rounds=[]`、`completed_unreleased_rounds=[]`、`forbidden_surface_hits=[]`、`duplicate_response_rounds=[]`、`returncode_fail_rounds=[]`。回归组合 `tests.process.test_relation_dialogue_smoke`、`test_terminal_input`、`test_terminal_ui`、`test_digital_entrypoint`、`test_model_expression` 与 stale lifecycle 单测共 46 tests OK。完成口径是：第 9 点已通过；下一步进入第 10 点总核验，而不是继续把第 9 点失败态当作当前阻断。

ITR-08 第一百零二段把 `engram_index.live_dialogue_turn_refs` 断链检查继续推进到 `/memory` 检查面。此前 `project_engram_index_from_live_turn` 已能把 `dialogue_turn_refs` 写入 `live_dialogue_turn_refs` 并同步 `life_state.memory_index`，但 `temp/05` 指出的「dialogue log 增长而 engram 无 live refs」断链只能在测试与文档里人工核对。现在 `engram_index.py#engram_live_turn_chain_inspection_snapshot` 会比较 `engram_index`、`life_state.memory_index` 与 `dialogue_memory_summary` 的 turn count，输出 `engram_live_turn_chain_alignment`、`live_dialogue_turn_ref_count` 与 `engram_live_turn_chain_closed`；`/memory#reconstructive_memory_summary_v0` 合并该快照并把 `engram_live_turn_chain` 计入 domain presence。完成口径是：`docs/real—live0/07_memory_engram_and_state_store.md` 的 live turn 记忆链断链检查可在 slash 检查面追溯；仍然只是 inspection-only 结构证据，不生成固定回答、不新增 system prompt、不把记忆字段释放成外显语言。第 2 点整体仍未完成，不能进入第 3 点记忆重构总设计。

ITR-08 第一百零一段把 emotion episode/regulation 继续推进到 pain/arousal/tension 分支、live turn 同轮刷新与 `/emotion` 检查面。此前 `emotion_episode.py` 仅两种 episode 标签且 `emotion_regulation.py` 固定 `hold_then_articulate`，`temp/03` 与 `temp/11` 指出的调节路线未分支缺口仍在。现在 `resolve_affective_episode_profile` 会按 pain_pressure、repair_followup、dream_residue_load、arousal、relationship_tension 分支 `episode_label`、`regulation_route`、`action_tendency` 与 `language_label_candidates`；`resolve_emotion_regulation_profile` 会按 episode route、fatigue、pain 与 recovery_path 分支 `regulation_mode`（含 `recovery_window_hold`、`dream_consolidation_hold`、`reappraisal_then_articulate`、`slowdown_then_confirm`）；`project_affective_episode_from_live_turn` 与 `project_emotion_regulation_from_live_turn` 在 `resident_turn_writeback.py` 同轮写出 body 文件并写入 terminal loop live 字段；`/emotion#emotion_regulation_summary_v0` 通过 `_emotion_regulation_branch_inspection_snapshot` 显示 route/mode/branch reason 与 live refresh。完成口径是：`docs/real—live0/03_body_affect_homeostasis.md` 要求的 episode→regulation 链具备多路线分支与 live 可追溯证据；仍然只是结构化调节证据，不生成情绪话术、不新增 system prompt、不把 regulation 字段释放成外显语言。第 2 点整体仍未完成，不能进入第 3 点记忆重构总设计。
