...
- “现有版本接下来按什么顺序补厚代码块” -> 回 `entry/v0_current_iteration_plan.md`
...
2. 一轮开发只带一个主合同包进入，不要把十四层都同时当作“当前要改的文档”。
...
- `package_specs/`：代码包规格层
- `code_blueprints/`：代码蓝图压缩层
- `code_scaffolds/`：代码脚手层
- `implementation_architecture/`：实现蓝图层
- `engineering_depth/`：工程深描层
- `动力学升级/`：schema → 数值动力学生命循环升级方案（见 `动力学升级/README.md`）

这次整理之后，后续如果有人觉得 `v0` “又乱了”，先不要新建目录；先回到上面这十五个柜位里判断应该并回哪一柜。
...
| 层 | 文件 | 作用 |
|---|---|---|
| 入口层 | `../README.md`、`README.md`、`v0_implementation_index.md`、`v0_delivery_status_board.md`、`v0_module_execution_catalog.md`、`v0_current_iteration_plan.md` | 说明入口、总索引、当前进度、模块执行目录和当前版本代码补厚计划 |
| 映射层 | `../mapping/README.md`、`../mapping/readme_block_engineering_realization_v0.md`、`../mapping/0_to_257_engineering_utilization_map.md`、`../mapping/theory_engineering_code_trace_matrix.md`、`../architecture/theory_closure_and_engineering_readiness_audit.md` | 把 `docs/README.md`、`00-258` 和非编号根文档压成工程 slice、runtime carrier、v0 工程文档、`life_v0` 代码包与 gate，并校核当前差距在理论还是工程 |
...
| 代码包规格层 | `package_specs/README.md`、`package_specs/01_life_v0_package_construction_matrix.md`、`package_specs/02_shared_object_write_authority_and_dependency_graph.md`、`package_specs/03_incremental_package_delivery_protocol.md` | 把代码总架构继续压到逐包施工：每个 `life_v0/*` 包读什么理论、首写什么对象、写什么 runtime、守什么测试和交付协议 |
...
为了避免“五个文档包”和“十四层分层”看起来像两套体系，直接按下面理解：

| 开工视角的文档包 | 在十四层分层里覆盖哪些层 |
|---|---|
| 入口包 | 入口层 |
| 回链包 | 映射层 |
| 主体合同包 | 总架构层、代码框架层、代码总架构层、代码包规格层、代码蓝图压缩层、代码脚手层、实现蓝图层、工程深描层、共用合同层、主体 slice 合同层 |
| 链尾与进程包 | 链尾桥接合同层、外层壳/进程合同层、外周参考层的一部分 |

所以：

1. 四包回答“这轮先带哪一柜进去”。
2. 十四层回答“这份文件在整个工程入口里站在哪一层”。
3. 先按五包开工，再用十四层防止写串层。
...
| 理论文档族最终由哪些工程文档、代码包和 gate 承接 | `mapping/theory_engineering_code_trace_matrix.md` |
