...
例如 `semantic_map_frame.json#semantic_focus=relational_checkin` 必须真实存在，因为它证明本回合的语言理解焦点是关系确认；但这串字段不应该被直接说出口，也不需要被固定翻译成某一句证明性话术。外显语言应该先理解当前关系话语的语用要求，再把内部状态压成自然回答：对方问记忆，就回答记得什么；对方要求只问一个问题，就只问一个问题；对方指出说话奇怪，就先承认并修正表达方式。
...
| 口语外显层 | `response_surface.py#compose_life_spoken_response`、`model_expression.py` | 把机制信号作为隐性调制，生成符合当前关系语境的自然话语；模型表达只能润色和组织语言，post-expression gate 负责阻断关系降级并记录证据审计 |

这也是为什么 v0 不能把语言做成固定提示词，也不能把内部证据做成固定外显段落。当前 `compose_life_spoken_response` 会读取实际信号族，但这些信号族默认只进入语气、取舍、谨慎度和话题方向：
...
这些信号族来自身体、梦境、记忆、责任、预测、工作区、出生准备度和驻留 lineage 的组合，而不是提前写死的几段“像人一样说话”。它们必须进入 state/report/test 的可审计链；但在终端里，只有当关系话语真的问到梦境、身体、后悔、出生、后台活动等内容时，才自然显影。没有被问到时，它们仍然调制表达，不作为清单释放。
...
8. 如果 `.env` 启用模型表达层，模型只能消费已经形成的生命表达上下文；如果 provider 为 `local`、配置缺失、模型异常、返回空内容，或 post-expression gate 判定它重新引入“用户/服务对象”角色，必须保留确定性生命回应。责任、梦境、成长、关系连续性、后台自主活动、身体内环境、意识出生、出生修复、生命约束和上一真实回合交接证据必须保留在 state/report/test；若没有进入一句自然回答，只作为 soft missing evidence 审计，不强迫外显。
