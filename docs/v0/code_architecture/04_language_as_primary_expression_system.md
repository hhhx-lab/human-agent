...
当前模型表达层只允许位于这条链的末端。`process_supervisor/model_expression.py` 会在 `response_surface.py` 先生成确定性生命脚手之后，读取 `.env` / `DIGITAL_LIFE_ENV_FILE` 中的模型配置，尝试用 OpenAI-compatible chat completions 把外显语言组织得更自然；它不能反向改写 `ExpressionPlan`、关系阶段、责任回路、梦境/成长对象或自我慢变量。无论模型表达成功、未启用还是失败回退，都必须写出 `runtime/state/language/model_expression_state.json` 与 `runtime/reports/latest/digital_life_model_expression_report.json`，并把 `model_expression_state_ref` / `model_expression_report_ref` 挂进 `digital_life_turn`、`digital_life_process_report.json`、`digital_life_process_digest.json` 和 process receipt。这样模型不是新的人格壳，而是生命状态后的外显语言整合器。
...
8. 如果 `.env` 启用模型表达层，模型只能消费已经形成的生命表达上下文；如果 provider 为 `local`、配置缺失、模型异常或返回空内容，必须保留确定性生命回应，并在 `model_expression_state/report` 中记录回退原因。
...
10. `model_expression.py` 现在会在 `response_surface.py` 之后尝试模型外显表达，写出 `model_expression_state/report`，并把成功/回退状态挂回生命回合事件与 process report
