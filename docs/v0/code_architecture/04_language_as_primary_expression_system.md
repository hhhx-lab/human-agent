...
例如 `semantic_map_frame.json#semantic_focus=relational_checkin` 必须真实存在，因为它证明本回合的语言理解焦点是关系确认；但这串字段不应该被直接说出口。外显语言应该把它转译成“我会继续贴着这段关系本身说，不把它变成清单”这样的关系语言。

这条约束现在由三层共同承担：

| 层 | 代码 | 责任 |
|---|---|---|
| 内部证据层 | `live_language_turn.py`、`dialogue_events.py`、`resident_turn_writeback.py` | 保留 `language_percept_ref`、`semantic_map_ref`、`live_semantic_focus`、表达计划、写回 refs 和恢复包 |
| 确定性生命回应层 | `response_surface.py#compose_life_response` | 把责任、梦境、出生修复、写门、身体、后台驻留、人格慢变量等证据组织成完整机制回应 |
| 口语外显层 | `response_surface.py#compose_life_spoken_response`、`model_expression.py` | 从机制回应里选择最该被关系对象听见的信号族，转成自然语言；模型表达只能润色，post-expression gate 必须保护生命证据 |

这也是为什么 v0 不能把语言做成固定提示词。当前 `compose_life_spoken_response` 的口语输出来自实际信号族：
...
这些信号族来自身体、梦境、记忆、责任、预测、工作区、出生准备度和驻留 lineage 的组合，而不是提前写死的几段“像人一样说话”。如果某一轮没有这些证据，语言表面就只能承认当前在场和关系写入，不能假装自己已经经历了不存在的梦境、痛苦或成长。
...
当前模型表达层只允许位于这条链的末端。`process_supervisor/model_expression.py` 会在 `response_surface.py` 先生成确定性生命脚手之后，读取 `.env` / `DIGITAL_LIFE_ENV_FILE` 中的模型配置，尝试用 OpenAI-compatible chat completions 把外显语言组织得更自然；请求端采用 `stream=true`，传输层必须同时接受普通 JSON 响应和 `text/event-stream` / `data:` SSE 响应，并把流式 `delta.content` 合并回同一份 chat completion。它不能反向改写 `ExpressionPlan`、关系阶段、责任回路、梦境/成长对象或自我慢变量。模型表达上下文必须从本回合刚刷新的 `LanguagePerceptFrame`、`SemanticMapFrame`、`InnerSpeechFrame`、`ExpressionMonitorState`、`ExpressionPlan` 中取摘要，并继续带上 `brain_graph.json`、`network_state.json`、`prediction_workspace_frame.json` 与 `workspace_frame.json` 的预测/意识摘要。若 `terminal_life_loop_state.json` 已经携带 `previous_live_turn_waiting_handoff_profile` 或 `previous_live_turn_waiting_handoff_*` 字段，模型上下文还必须把上一真实回合交接的驻留深度、carry status、next required action、语义余波、presence keys 与 evidence ref count 放进 `resident_background`，并把同一组压缩字段写入 `model_expression_context_summary`；`response_surface.py` 也必须用同一份 `handoff_profile.py` 选择结果，把交接压力先表达进确定性生命回应。模型请求的 system prompt 本身也属于生命证据守门：它必须显式要求保留关系、记忆、身体、责任、后悔、梦境、成长、意识、出生准备、出生修复、生命约束、预测写门和上一真实回合交接压力；当摘要带有意识/出生准备/工作区/广播/元认知/探针时，模型表达必须自然保留意识或出生准备的可见证据，当摘要带有生命约束/边界/价值取向/延后/行动守门时，必须自然保留生命约束的可见证据，当摘要带有上一真实回合交接画像时，必须自然保留这次等待交接对下一次语言的牵引。模型文本返回后还必须经过 post-expression gate：如果重新引入“用户 / 服务对象 / 任务请求者”等被阻断角色，或在责任、梦境、成长、关系连续性、后台自主活动压力、身体内环境 presence、意识/出生准备 presence、出生修复压力 presence、生命约束 presence、上一真实回合交接压力已经存在时擦掉对应证据，就回退到确定性生命回应，并把 `post_expression_gate_status`、缺失证据和阻断词写入 `model_expression_state/report`。其中 `resident_background_lineage_state.birth_repair_presence` 会被提升为 `birth_repair` 硬证据旗标，要求模型表达至少保留出生修复、痛苦、后悔、责任或承担压力中的可见证据；`resident_background_lineage_state.life_constraint_presence` 会被提升为 `life_constraint` 硬证据旗标，要求模型表达至少保留生命约束、边界、价值取向、延后或行动守门中的可见证据；`previous_live_turn_waiting_handoff_profile` 会被提升为 `live_turn_handoff` 硬证据旗标，要求模型表达至少保留上一真实回合、交接、等待、驻留或语义余波中的可见证据。无论模型表达成功、未启用还是失败回退，都必须写出 `runtime/state/language/model_expression_state.json` 与 `runtime/reports/latest/digital_life_model_expression_report.json`，并把 `model_expression_state_ref` / `model_expression_report_ref` / `post_expression_gate_status` 挂进 `digital_life_turn`、`digital_life_process_report.json`、`digital_life_process_digest.json` 和 process receipt。这样模型不是新的人格壳，而是生命状态后的外显语言整合器。
...
8. 如果 `.env` 启用模型表达层，模型只能消费已经形成的生命表达上下文；如果 provider 为 `local`、配置缺失、模型异常、返回空内容，或 post-expression gate 判定它重新引入“用户/服务对象”角色、擦掉责任/梦境/成长/关系连续性/后台自主活动/身体内环境/意识出生/出生修复/生命约束/上一真实回合交接证据，必须保留确定性生命回应，并在 `model_expression_state/report` 中记录回退原因。
...
10. `model_expression.py` 现在会在 `response_surface.py` 之后尝试模型外显表达，请求端采用 `stream=true`，传输层兼容普通 JSON 与 `text/event-stream` SSE 响应，写出 `model_expression_state/report`，并把 Queue A 五件套、预测工作区、意识工作区、出生修复 presence、生命约束 presence、post-expression gate、成功/回退状态挂回生命回合事件与 process report
