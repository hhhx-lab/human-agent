...
8. 如果 `.env` 启用模型表达层，模型只能消费已经形成的生命表达上下文；如果 provider 为 `local`、配置缺失、模型异常、返回空内容，或 post-expression gate 判定它重新引入“用户/服务对象”角色、擦掉责任/梦境/成长/关系连续性证据，必须保留确定性生命回应，并在 `model_expression_state/report` 中记录回退原因。
