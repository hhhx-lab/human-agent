...
例如 `semantic_map_frame.json#semantic_focus=relational_checkin` 必须真实存在，因为它证明本回合的语言理解焦点是关系确认；但这串字段不应该被直接说出口，也不需要被固定翻译成某一句证明性话术。外显语言必须由模型在结构化生命上下文之后形成，并通过 post-expression gate；没有通过时，内部状态只进入审计材料和报告，不被程序拼成回答。
...
| 结构化审计材料层 | `response_surface.py#compose_life_response` | 把责任、梦境、出生修复、写门、身体、resident 驻留、人格慢变量等证据组织成 `audited_expression_material_v0`，不生成自然语言回退 |
| 口语外显层 | `model_expression.py` | 只在模型表达通过 post-expression gate 后释放自然话语；`compose_life_spoken_response` 在无模型时保持未释放，防止程序固定拼句 |

这也是为什么 v0 不能把语言做成固定提示词，也不能把内部证据做成固定外显段落。当前无模型或模型被 gate 拒绝时，外显语言保持未释放；实际信号族只进入 state/report/test 和模型表达上下文：
...
8. 如果 `.env` 启用模型表达层，模型只能消费已经形成的生命表达上下文；如果 provider 为 `local`、配置缺失、模型异常、返回空内容，或 post-expression gate 判定关系降级，本轮自然语言必须保持未释放。责任、梦境、成长、关系连续性、resident 自主活动、身体内环境、意识出生、出生修复、生命约束和上一真实回合交接证据必须保留在 state/report/test；若没有进入模型外显文本，只作为 soft missing evidence 审计，不强迫外显。
...
10. `model_expression.py` 现在会在 `response_surface.py` 之后尝试模型外显表达，请求端采用 `stream=true`，传输层兼容普通 JSON 与 `text/event-stream` SSE 响应，写出 `model_expression_state/report`，并把 Queue A 五件套、预测工作区、意识工作区、出生修复 presence、生命约束 presence、post-expression gate、成功/未释放状态挂回生命回合事件与 process report
