...
3. 让 `response_surface.py` 真实消费 `relationship_timeline.json`
4. 让 `resident supervision / terminal loop` 带回 `commitment_expression_plan.json`
5. 让 `apology_repair_language_trace.json` 进入更厚的长期回合恢复与关系修复外显
