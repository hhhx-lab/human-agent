...
6. `process closeout / process report / resident governance snapshot` 现在也已继续显式回链：
   - `relationship_timeline_ref`
   - `commitment_expression_plan_ref`
   - `apology_repair_language_trace_ref`
   - `long_horizon_language_refs`
7. 下一步转向把这批长期语言对象继续接进 `terminal_life_loop_state` 的更厚 waiting continuity，以及后台 resident governance 的长期节律治理
