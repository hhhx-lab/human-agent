...
| 确认证据 | `digital_life_waiting_heartbeat.json`、`runtime/state/terminal/idle_strategy_state.json`、`runtime/state/terminal/resident_governance_state.json`、`runtime/state/terminal/resident_governance_snapshot.json`、`digital_life_resident_governance_report.json`、`digital_life_process_report.json`、incident / relaunch recovery reports |
...
3. `process_supervisor/`，优先继续补 waiting governance 的字段贯通、resident governance 运行态 / closeout 双层治理，以及 heartbeat 节律实化
