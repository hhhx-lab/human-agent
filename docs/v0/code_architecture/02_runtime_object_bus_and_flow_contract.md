...
| 对象 | 首写器官 | 主要消费者 | 当前 / 下一步文件 | 主要证据 |
|---|---|---|---|---|
| `ActionCandidateSet` | `membrane/candidate_arena.py` | `validators`、`schema_runner`、`shell_command`、`process_supervisor` | 已存在；已接 `shadow_gate.py`，下一步补 `action_intent_bridge.py` | `runtime/state/action/*`、`world_contact_audit_report.json` |
