...
| `ModelExpressionState` | `process_supervisor/model_expression.py` | `live_turn_cycle`、`dialogue_events.py`、`resident_turn_writeback`、`process_report.py` | 已接入；在确定性 `response_surface.py` 回应之后按 `.env` 尝试模型外显表达，当前上下文摘要已消费 Queue A 五件套、预测工作区、意识工作区、脑图和网络状态；模型文本还要经过 post-expression gate，阻断“用户/服务对象”等角色回流和责任/梦境/成长/关系/后台自主活动证据擦除，成功/跳过/回退都写成可追踪状态 | `runtime/state/language/model_expression_state.json`、`runtime/reports/latest/digital_life_model_expression_report.json`、`digital_life_turn#model_expression_state_ref`、`digital_life_turn#post_expression_gate_status` |
