...
| `ModelExpressionState` | `process_supervisor/model_expression.py` | `live_turn_cycle`、`dialogue_events.py`、`resident_turn_writeback`、`process_report.py` | 已接入；在结构化 `response_surface.py` 审计材料之后按 `.env` 尝试模型外显表达，当前上下文摘要已消费 Queue A 五件套、预测工作区、意识工作区、脑图和网络状态；模型文本还要经过 post-expression gate，阻断“用户/服务对象”等角色回流和责任/梦境/成长/关系/resident 自主活动证据擦除，成功/跳过/未释放都写成可追踪状态 | `runtime/state/language/model_expression_state.json`、`runtime/reports/latest/digital_life_model_expression_report.json`、`digital_life_turn#model_expression_state_ref`、`digital_life_turn#post_expression_gate_status` |
...
| 对象 | 首写器官 | 主要消费者 | 当前 / 下一步文件 | 主要证据 |
|---|---|---|---|---|
| `MemoryWriteGate` | `state_store/memory_write_gate.py` | `state_merge_guard.py`、`live_turn_cycle.py`、`idle_strategy.py`、`background_continuity.py`、`background_lineage_state.py`、`dialogue_events.py`、`resident_turn_writeback.py`、`terminal_loop/dialogue_writeback.py`、`process_report.py`、`response_surface.py` | 已存在；保持候选写入事务、validation envelope、隔离路线和长期治理 ref；当前 `body_signal_write_modulation` 已能穿过 waiting、lineage、live turn、writeback bundle、resumed packet、process report/digest/receipt | `runtime/state/memory/memory_write_gate.json`、`runtime/reports/latest/dialogue_writeback_bundle.json#resident_background_lineage_body_signal_refs`、`runtime/reports/latest/digital_life_process_digest.json#body_signal_ref_set`、`runtime/receipts/digital_life_process_<run_id>.json#body_signal_ref_set` |
| `MemoryRetrievalFrame` | `state_store/memory_retrieval.py` | `live_turn_cycle.py`、`dialogue_events.py`、`resident_turn_writeback.py`、`idle_strategy.py`、`background_lineage_state.py`、`response_surface.py`、`model_expression.py`、`life_state.py` | 已存在；当前把当前话语、语言五件套、engram、关系记忆、退出梦境摘要、自传栈、责任链和合并门压成 cue-driven reconstructive recall 面，再把 activated refs、tiered recall、reconstruction focus 和 blocked refs 送进语言审计、模型表达、回合事件、写回包、状态根与后台谱系 presence | `runtime/state/memory/memory_retrieval_frame.json`、`runtime/state/life_state.json#memory_index.memory_retrieval_refs`、`runtime/reports/latest/dialogue_writeback_bundle.json#memory_retrieval_writeback_refs`、`runtime/reports/latest/dialogue_writeback_bundle.json#resident_background_lineage_memory_retrieval_refs`、`runtime/state/terminal/resident_background_lineage_state.memory_retrieval_presence`、`runtime/state/language/model_expression_state.json#model_expression_context_summary.memory_retrieval_frame_ref` |
...
external utterance
  -> LanguagePerceptFrame / SemanticMapFrame
  -> MemoryRetrievalFrame
  -> dialogue_events.memory_retrieval_*
  -> audited expression material
  -> model expression context summary
  -> dialogue_writeback_bundle.memory_retrieval_writeback_refs
  -> terminal_life_loop_state.memory_retrieval_presence_profile
  -> resident_background_lineage_state.memory_retrieval_presence
  -> next dialogue event / resumed dialogue packet / response surface
  -> life_state.memory_index.memory_retrieval_refs
```

如果 `memory_retrieval_frame.json` 只在 `state_store` 包中生成，而 `dialogue_events.py`、`response_surface.py`、`model_expression.py`、`resident_turn_writeback.py`、`background_lineage_state.py` 和 `life_state.py` 不读取它，说明记忆仍停在索引层，没有进入关系语言的重构和跨唤醒连续体。
