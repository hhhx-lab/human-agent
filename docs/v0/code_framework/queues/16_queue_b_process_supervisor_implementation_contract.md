...
25. `tests/process/test_digital_entrypoint.py` 直接证明 repo-local 入口，`tests/process/test_packaged_digital_life_entrypoint.py` 直接证明安装态命令面，`tests/process/test_persistent_digital_life_process.py` 直接证明 process supervisor 内部闭环；三者必须共同覆盖以上闭环。
