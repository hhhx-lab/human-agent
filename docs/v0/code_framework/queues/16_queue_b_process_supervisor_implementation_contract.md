...
54. S05/S09 的 `life_constraint_profile` 必须进入常驻等待治理和真实回合写回链：`idle_strategy.py` 从 `schema_cross_file_logic.json` 与 `run_manifest.json` 生成 `life_constraint_refs / queue_e_cross_layer_gate_status / life_constraint_waiting_posture / attention_target / attention_reason`；`heartbeat.py` 写入 waiting heartbeat、resident governance 和 terminal loop；`dialogue_events.py` 必须在 `digital_life_turn` 中写出同一组 fields 与 `life_constraint_evidence_refs`；`resident_turn_writeback.py` 必须让 `dialogue_writeback_bundle.life_constraint_refs`、总 `resident_background_lineage_refs` 与 `resumed_external_dialogue_packet.life_constraint_*` 保留同一组 schema refs 和约束 refs；`response_surface.py` 必须表达生命约束等待姿态、焦点、理由和证据数量。
