...
25. `resident_supervision.py` 必须在 bootstrap continuity refresh 后写出 `background_convergence_summary.json`；`background_continuity.py` 必须在下一次唤醒时把它恢复成 `background_convergence_summary_ref` 与 `background_convergence_*` 字段；`idle_strategy.py` / `heartbeat.py` / `process_report.py` / receipt 必须把它作为跨进程关系阶段与慢变量收敛证据继续回链。
