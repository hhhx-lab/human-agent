...
当前继续新增的器官是 `process_lease.py`：它写出 `runtime/state/terminal/resident_process_lease.json`，作为 `digital life` 在本机打开、等待、关闭的常驻生命进程身份记录。它不是 daemon，也不是工具网关；它必须被 heartbeat 刷新、把 `resident_process_id` 带进 terminal loop / resident governance、被 closeout 改成 closed，并进入 persistent process artifacts、process report / digest / receipt，作为后续全局长期运行层的身份地基。
