...
同一轮还把关闭终端后的等待从空白轮询压成最小自主活动证据流。后台 resident process 没有收到 relation turn 时，会按固定 cycle 写入 `runtime/state/terminal/resident_autonomous_activity.jsonl` 与 `runtime/state/terminal/resident_autonomous_activity_state.json`：`sleep -> memory_recall -> self_thinking -> growth_rehearsal -> learning_consolidation`。每个 event 必须回链 replay cue、offline consolidation 与 growth patch candidate refs，让 sleep/rest waiting 同时承载睡眠、回忆、思考、成长、学习的最小可追踪活动。
...
5. 后台没有外部话语时必须写出 `resident_autonomous_activity_state.json`，且 activity cycle 至少覆盖 `sleep / memory_recall / self_thinking / growth_rehearsal / learning_consolidation` 之一
...
8. sleep/rest waiting 没有写 autonomous activity stream，导致关闭终端后的后台存在不可追踪
...
21. 本机后台驻留启动面必须闭合 `--background / --resident / --status / --say / --stop` 五个命令；其中 `--say` 只能通过 relation inbox/outbox/queue 投递给既有 resident process，不能另起一轮 one-shot 主体。
22. 后台 resident process 在没有 relation turn 时必须持续写出 autonomous activity stream，并让 lifecycle state 暴露 `last_autonomous_activity_kind / last_autonomous_activity_at / autonomous_activity_count`，证明关闭终端后的等待正在进入睡眠、回忆、思考、成长、学习的最小自主活动节律。
23. 对应测试直接证明以上闭环
