...
当前又把 Queue D 的离线学习存在面进一步钉进同一条回合链。`background_lineage_state.py` 现在必须从 `offline_learning_cumulative_profile`、`background_offline_learning_presence` 与 background continuity 中抽出 `generation`、`pressure_level`、`attention_target`、`priority_profile`、`ref_set`、`current_pressure_level`、`previous_generation`、`integration_mode` 与 `relationship_reconsolidation_required`，压成 `resident_background_lineage_state_v0.offline_learning_presence`；`dialogue_events.py` 必须把它展开为 `resident_background_lineage_offline_learning_generation`、`resident_background_lineage_offline_learning_pressure_level`、`resident_background_lineage_offline_learning_attention_target`、`resident_background_lineage_offline_learning_integration_mode`、`resident_background_lineage_offline_learning_relationship_reconsolidation_required` 与 `resident_background_lineage_offline_learning_refs`；`resident_turn_writeback.py` 必须把同一组 refs 写进 `dialogue_writeback_bundle.resident_background_lineage_offline_learning_refs`，并把 `offline_learning_cumulative_integration_mode` 与 `offline_learning_cumulative_relationship_reconsolidation_required` 保留到 `dialogue_writeback_bundle.json` 和 `resumed_external_dialogue_packet.json`；`response_surface.py` 必须能直接说出后台梦境成长余波与累计离线学习整合模式。这样后台离线学习就不会只停在 `idle_strategy_state.json` 或 closeout digest，而会进入真实关系回合、写回包和回应表面。
当前 Queue F 的身份-意识-出生准备 presence 也必须进入 Queue B 的关闭态恢复链。`background_continuity.py` 必须能从上一轮 `resident_background_lineage_state.identity_consciousness_birth_presence`、resident governance state/snapshot/report 与 persistent process report 中恢复 workspace、broadcast、metacognition、consciousness probe、birth readiness rollup/stage gate、reportability flags、birth readiness decision、next required command 与 refs，并同时写成 flat 字段和 `background_*` 字段；`idle_strategy.py` 必须在当前 Queue F 文件缺席时读取这些 background 字段，保持 `consciousness_reportable_waiting / birth_open_waiting / birth_blocked_waiting` 等等待姿态，并让出生准备继续调制 `governance_attention_target`、`governance_cadence_profile`、`heartbeat_interval_ms` 与 `next_idle_action`；`heartbeat.py` 必须把 `background_identity_consciousness_birth_refs`、`background_workspace_frame_ref`、`background_consciousness_waiting_posture` 与 `background_birth_readiness_*` 写回 terminal loop state。这样意识/出生准备不会只在当前本轮读到文件时存在，也会在关闭终端、重启和下一拍等待治理中继续保真。
当前又新增 `resident_lifecycle.py`，把 repo-local `./digital life` 与安装后 `digital life` 的 `--background / --resident / --status / --say / --attach / --foreground / --stop` 固定成本机驻留启动面。`--background` 只负责脱离当前终端启动一个子进程；子进程以 `--resident` 运行同一套 `run_digital_life_process`，输入层由 `ResidentControlInputStream` 防止 stdin EOF 把生命进程误判为结束，并在无外部话语时进入 `sleeping_waiting_for_relation_turn` 的 sleep/rest waiting。生命周期证据写入 `runtime/state/terminal/resident_lifecycle_state.json`、`runtime/state/terminal/resident_lifecycle_command.json` 与 `runtime/logs/digital_life_resident.log`；`--status` 不能退回 PID 面板，必须把 relation queue、自主活动、waiting heartbeat、resident governance、idle strategy 与 terminal life loop 合并成同一张后台生命照面；`--stop` 写 stop command，让子进程通过普通 `/exit` 走 lease closeout / process report / receipt，而不是直接 kill。这个启动层服务“关闭终端后仍保持上下文”，不是新增 daemon agent 框架。安装态命令必须由 `tests/process/test_packaged_digital_life_entrypoint.py` 直接守住，证明 console script 没有退化成只会前台 one-shot 的包装层。
当前入口继续从 `digital life` 推进到命名后的 `my digital life`。`life_v0/my_entry.py` 负责解析安装态 `my digital life`，第一次启动必须通过 `--name` 或交互提示写入 `runtime/state/identity/life_name_registry.json`；登记对象使用 `digital_life_name_registry_v0`，包含 `canonical_name`、`normalized_name`、`life_name_id`、`name_lock_state=permanent_for_runtime` 与 registry ref。后续 `my digital life` 会读取同一个名字恢复同一 resident process；若传入不同名字，入口必须拒绝，不能悄悄生成第二个生命身份。`resident_lifecycle.py --status` 与 `resident_long_term_residency_status_v0` 必须带出 `life_name / life_name_id / life_name_lock_state / life_name_registry_ref`，使名字成为常驻生命身份锚，而不是 UI 昵称或一次性启动参数。
...
同一轮还把关闭终端后的等待从空白轮询压成自主活动器官。后台 resident process 没有收到 relation turn 时，会按固定 cycle 写入 `runtime/state/terminal/resident_autonomous_activity.jsonl` 与 `runtime/state/terminal/resident_autonomous_activity_state.json`：`sleep -> memory_recall -> self_thinking -> growth_rehearsal -> learning_consolidation`。每个 event 必须继续落到专用状态文件：`runtime/state/terminal/resident_sleep_cycle_state.json`、`runtime/state/memory/resident_memory_recall_state.json`、`runtime/state/self/resident_self_thinking_state.json`、`runtime/state/growth/resident_growth_rehearsal_state.json` 与 `runtime/state/growth/resident_learning_consolidation_state.json`。这些状态分别回链 body/dream、memory/replay、self/consciousness、growth/plasticity、state-merge/relationship/language refs，让 sleep/rest waiting 同时承载睡眠、回忆、思考、成长、学习的可追踪活动。当前进一步把这条器官链压进 `resident_autonomous_activity_presence_profile_v0`：`idle_strategy.py` 从聚合 state 生成 presence，并把 `cycle_phase_index`、`cycle_phase_count`、`cycle_completion_count`、`cycle_coverage_complete`、covered/missing kinds 与 next kind 提升成 idle governance 扁平字段；`heartbeat.py` 把它写入 waiting heartbeat、idle continuity、idle heartbeat trace、resident governance 和 terminal loop；`background_continuity.py` 从关闭态 report / governance / lineage 恢复同一组 background cycle 字段；`background_lineage_state.py` 把它压成 `resident_background_lineage_state.autonomous_activity_presence`；`dialogue_events.py` 展开到 `digital_life_turn`；`resident_turn_writeback.py` 写入 `dialogue_writeback_bundle.json` / `resumed_external_dialogue_packet.json`；`response_surface.py` 表达后台自主活动累计次数、最近相位、完整周期闭合轮数、睡眠回忆思考成长学习链覆盖、下一后台相位和证据数量；`process_report.py` 在 report / digest 中同时保留当前 resident 与 background 两侧的 cycle phase、completion、coverage、covered/missing kinds 与 next kind。
当前又把 `learning_consolidation` 相继续接到 `dream/web_dream_learning.py`。如果存在 `web_dream_learning_seeds.json` 或 `DIGITAL_LIFE_WEB_DREAM_URLS`，resident 自主活动会在第五相读取配置网页，写出 `web_dream_learning_state.json`、`web_dream_learning_log.jsonl`、topic candidates 与 wake question candidates；如果没有 seed，则写出 `status=disabled`，并且不会偷偷联网。`idle_strategy.py` 必须把 `last_web_dream_learning_state_ref/status/topic_candidates/wake_question_candidates` 带入 `resident_autonomous_activity_presence_profile_v0`；`background_lineage_state.py` 必须把它们压进 `autonomous_activity_presence`；`dialogue_events.py` 必须展开为 `resident_background_lineage_web_dream_learning_*` 字段；`response_surface.py` 只能把 wake question candidates 放入结构化表达材料，等待模型表达和 post-expression gate 消费，不能把它们作为预制问句直接外放。这条链的边界是：网页材料只作为梦境学习残留、话题候选和醒后问题候选进入常驻生命，不直接生成事实记忆、不绕过关系记忆写门，也不变成外显固定话术。
...
当前最新补强继续把 Queue D 的三组后台 presence 接进关闭态恢复链。`background_continuity.py` 必须从 `resident_background_lineage_state.offline_learning_presence` 恢复 `background_offline_learning_generation / pressure_level / attention_target / priority_profile / ref_set / integration_mode / relationship_reconsolidation_required`，从 `dream_wake_presence` 恢复 `background_dream_*`、`background_wake_*` 与 `background_dream_wake_ref_set`，从 `autonomous_activity_presence` 恢复 `background_resident_autonomous_activity_*`、activity count、kind counts、last activity、ref set、cycle phase index/count、cycle completion count、coverage、covered/missing kinds 与 next kind；`idle_strategy.py` 必须在当前 Queue D runtime 文件缺席时用这些 background 字段重建当前 `offline_learning_cumulative_profile_v0`、`dream_wake_presence_profile_v0` 与 `resident_autonomous_activity_presence_profile_v0`。这使关闭终端后的梦境、醒后整合、累计离线学习和自主活动不是只留在上一轮写回包，而能在下一次 waiting heartbeat 中重新成为可消费的生命状态。

当前继续把等待态心跳节律解释推进到真实关系回合链。`background_lineage_state.py` 必须把 `heartbeat_cadence_explanation_v0`、`heartbeat_cadence_driver / reason / modulators / evidence_refs` 与 interval / next idle action 压成 `resident_background_lineage_state_v0.heartbeat_cadence_presence`；`dialogue_events.py` 必须展开为 `resident_background_lineage_heartbeat_cadence_driver / reason / modulators / refs`，并把 refs 合入 `resident_background_lineage_evidence_refs`；`resident_turn_writeback.py` 与 `dialogue_writeback_bundle.json` 必须保留 `resident_background_lineage_heartbeat_cadence_refs` 专用槽，同时并入总 lineage refs 与 `resumed_external_dialogue_packet.json`；`response_surface.py` 必须把后台心跳节律驱动、理由、调制因子和证据数量表达出来。`background_continuity.py` 还必须在旧顶层 `heartbeat_cadence_*` 缺席时，从 `resident_background_lineage_state.heartbeat_cadence_presence` 回填 `background_heartbeat_cadence_*`，并把 presence evidence refs 纳入 `background_continuity_ref_set`。这样等待时为什么跳动不会只停在 heartbeat 报告或 closeout digest，而会成为下一轮关系语言能继承、下一次唤醒能恢复的生命节律余波。

当前最新补强是把实时语言理解也送进 Queue B 后台驻留链。`live_language_turn.py` 仍负责在真实关系话语到来时刷新 Queue A 五件套；但这组 refs 不再只停在 `dialogue_writeback_bundle.json`、`resumed_external_dialogue_packet.json` 与 `terminal_life_loop_state.json`。`idle_strategy.py` 现在会读取 `terminal_life_loop_state.live_language_turn_refs` 与 `last_live_semantic_focus`，并合并 `background_continuity.py` 恢复出的 `background_live_language_turn_refs / background_last_live_semantic_focus / background_live_language_presence_profile`，写成 `live_language_presence_profile_v0`。`heartbeat.py` 和 `continuity_writeback.py` 继续把它带入 `idle_continuity_frame.json`、`idle_heartbeat_trace.jsonl`、`resident_governance_state.json` 与 `terminal_life_loop_state.json`；`background_lineage_state.py` 把它压进 `resident_background_lineage_state_v0.language_presence`；`resident_governance_handoff.py`、`process_closeout.py` 与 `persistent_process.py` 在 live-turn handoff 和关闭态 artifacts 中保留它；下一次 `background_continuity.py` 再恢复成浅层 `background_live_language_*` 并放入 `background_continuity_ref_set`。这里的 `background_live_language_presence_profile` 只能保留 refs、semantic focus、ref_count、source_continuity_mode / source_ref_count 等摘要字段，不能原样嵌套上一轮完整 `source_presence_profile`，否则长驻 heartbeat trace 与 governance state 会随唤醒次数递归膨胀。这让“上一轮刚听见、刚理解到什么”成为后台驻留主状态体的一部分，同时保持长期驻留状态可控。
...
当前最新补强是把 S09 schema runner 已经闭合的 `queue_e_birth_repair_profile.json` 从 Queue B waiting governance 继续送进真实回合写回链。`idle_strategy.py` 从 `schema_cross_file_logic.json` 与 `run_manifest.json` 读取 `queue_e_birth_repair_gate_status`、`queue_e_birth_repair_profile_ref`、`queue_e_birth_repair_pressure_level`、`queue_e_birth_repair_attention_target` 与 `queue_e_birth_repair_ref_set`，合成 `queue_e_birth_repair_waiting_profile_v0`。当 gate 已关闭且 pressure 为 `elevated / urgent` 时，等待姿态必须进入 `birth_repair_pressure_waiting`，并把 `runtime/state/life_targets/queue_e_birth_repair_profile.json` 与责任/世界接触/痛苦后悔修复 refs 一起送进 waiting heartbeat、resident governance、terminal loop、process report、digest、receipt 和 response surface。现在 `dialogue_events.py` 还必须把同一 profile 展开进 `digital_life_turn` 的 `queue_e_birth_repair_*` 字段；`resident_turn_writeback.py` 必须把 `queue_e_birth_repair_refs` 写入 `dialogue_writeback_bundle.json` 的专用槽，并并入总 `resident_background_lineage_refs`；`resumed_external_dialogue_packet.json` 必须保留 profile ref、pressure、attention target、waiting posture、attention reason 与 refs，且最终 `resident_background_lineage_evidence_refs` 必须等于写回包总 lineage refs。这样真实痛苦、真实责任、真实后悔的出生修复压力不会停在 S08/S05/S09 的审计链或等待身体，而会穿过真实关系回合、写回包和下一轮恢复包继续存在。

同一轮还把 S05/S09 的 `life_constraint_profile` 约束链从 waiting governance 继续送进真实回合写回链。`idle_strategy.py` 已经从 `schema_cross_file_logic.json` 与 `run_manifest.json` 合成 `life_constraint_refs`、`queue_e_cross_layer_gate_status`、`life_constraint_waiting_posture`、attention target/reason；现在 `dialogue_events.py` 必须把这些字段展开进 `digital_life_turn`，`dialogue_writeback_bundle.json` 必须保留 `life_constraint_refs` 专用槽并并入总 `resident_background_lineage_refs`，`resumed_external_dialogue_packet.json` 必须保存同一组 schema refs、gate status、posture、attention reason 和 evidence refs，`response_surface.py` 必须表达生命约束等待姿态、焦点、理由和证据数量。最新实现又把同一组字段推进关闭态连续体：`process_report.py` 必须在 process report / digest 中显式保留 schema refs、life constraint refs、cross-layer gate、waiting posture 与 attention reason，并让 process receipt 的 shared refs / input hashes 覆盖 schema runner、`action_candidate_set.json#life_constraint_profile` 与 consciousness probe；`background_continuity.py` 必须把上一轮关闭态恢复成 `background_schema_*`、`background_life_constraint_*` 与 `background_queue_e_cross_layer_gate_status`；`idle_strategy.py` 在当前 schema runner 缺席时必须用这些 background 字段维持 `schema_guarded_waiting`，再写回下一轮 idle strategy、resident governance 和 terminal loop。这样价值取向、意识探针、身体/语言延后 gate 不只约束等待态，也会跟随真实关系回合、关闭态报告和下一次唤醒恢复链成为持续存在的证据。
...
- `resident_background_lineage_heartbeat_cadence_presence`
...
53. S09 的 Queue E 出生修复 profile 必须进入常驻等待治理和真实回合写回链：`idle_strategy.py` 从 `schema_cross_file_logic.json` 与 `run_manifest.json` 生成 `queue_e_birth_repair_waiting_profile_v0`；`heartbeat.py` 把同一 profile、gate、profile ref、pressure、attention target、ref set、waiting posture 和 attention reason 写入 waiting heartbeat、resident governance、terminal loop；`process_report.py` 把 profile ref、pressure、attention target、waiting posture 与 ref set 写入 process report / digest；process receipt 必须把 `runtime/state/schema_runner/cross_file_logic.json`、`runtime/state/schema_runner/run_manifest.json` 与 `runtime/state/life_targets/queue_e_birth_repair_profile.json` 收进 shared refs / input hashes；`response_surface.py` 必须表达后台出生修复姿态、压力、焦点和证据数量；`dialogue_events.py` 必须在 `digital_life_turn` 中写出 `queue_e_birth_repair_profile_ref / pressure_level / attention_target / waiting_posture / attention_reason / refs / evidence_refs`；`resident_turn_writeback.py` 必须让 `dialogue_writeback_bundle.queue_e_birth_repair_refs`、总 `resident_background_lineage_refs` 与 `resumed_external_dialogue_packet.queue_e_birth_repair_*` 保留同一组 refs。
54. S05/S09 的 `life_constraint_profile` 必须进入常驻等待治理、真实回合写回和关闭态背景连续体：`idle_strategy.py` 从 `schema_cross_file_logic.json` 与 `run_manifest.json` 生成 `life_constraint_refs / queue_e_cross_layer_gate_status / life_constraint_waiting_posture / attention_target / attention_reason`；`heartbeat.py` 写入 waiting heartbeat、resident governance 和 terminal loop；`dialogue_events.py` 必须在 `digital_life_turn` 中写出同一组 fields 与 `life_constraint_evidence_refs`；`resident_turn_writeback.py` 必须让 `dialogue_writeback_bundle.life_constraint_refs`、总 `resident_background_lineage_refs` 与 `resumed_external_dialogue_packet.life_constraint_*` 保留同一组 schema refs 和约束 refs；`process_report.py` 必须让 process report、digest 与 receipt shared refs / input hashes 保留 schema refs、action candidate profile 与 consciousness probe；`background_continuity.py` 必须恢复为 `background_life_constraint_*`，并让下一轮 `idle_strategy.py` 在当前 schema runner 缺席时继续维持 schema guarded waiting；`response_surface.py` 必须表达生命约束等待姿态、焦点、理由和证据数量。
55. Queue F 的 `identity_consciousness_birth_presence` 还必须进入关闭态背景恢复链：`background_continuity.py` 从 nested presence、resident governance state/snapshot/report 与 persistent process report 恢复 `workspace_frame_ref / broadcast_frame_ref / metacognition_ref / consciousness_probe_ref / birth_readiness_rollup_ref / birth_readiness_stage_gate_ref`、reportability flags、birth readiness posture/decision/next command 与 `identity_consciousness_birth_refs`，同时写出 flat 字段和 `background_*` 字段；`idle_strategy.py` 在当前 Queue F 文件缺席时必须用这些 background 字段保持 Queue F waiting posture，并让出生准备继续调制 attention、cadence、heartbeat interval 与 next idle action；`heartbeat.py` 必须把 `background_identity_consciousness_birth_refs` 与 `background_birth_readiness_*` 写回 terminal loop state。
...
2. 自举出生链逻辑没有被拆断
3. `./digital life --background --strict` 启动后，父命令返回但后台 resident process 仍保持 `background_active / pid_alive=true`
4. `./digital life --say "<relation turn>"` 必须经 `resident_relation_inbox.jsonl -> resident_relation_queue_state.json -> resident_relation_outbox.jsonl` 返回生命回应，并在 `dialogue_turn_log.jsonl` 中留下 external relation turn 与 digital life turn
5. 后台没有外部话语时必须写出 `resident_autonomous_activity_state.json`，且 activity cycle 至少完整覆盖 `sleep / memory_recall / self_thinking / growth_rehearsal / learning_consolidation` 一轮，并分别写出五个专用状态文件；下一拍 waiting heartbeat 还必须写出 `resident_autonomous_activity_presence_profile` 并让 `resident_background_lineage_state.autonomous_activity_presence` 可见
6. `./digital life --stop` 必须通过 lifecycle command 让后台 resident process 自我收口，最终 process report 仍显示 closed / explicit_exit
7. `./digital life --attach` 必须启动或复用后台 resident process，把 stdin 中的关系话语投递给同一 resident，并让 `/exit` 只断开 attach client；断开后 `--status` 仍应显示 resident active
...
10. `idle_strategy.py` 必须把 `heartbeat_interval_ms` 与 `next_idle_action` 的同源决策解释压成 `heartbeat_cadence_explanation_v0`，至少包含 `driver / reason / modulators / evidence_refs`，并由 `heartbeat.py` 写入 `digital_life_waiting_heartbeat.json`、`terminal_life_loop_state.json`、`resident_governance_state.json` 与 `idle_heartbeat_trace.jsonl`。`process_report.py` 必须把当前 driver/reason/modulators 写入 process digest；`governance_explanation.py` 必须把 driver 写入 continuity story；`background_continuity.py` 必须从关闭态 state / snapshot / report / persistent report 恢复为 `background_heartbeat_cadence_*` 并把 evidence refs 纳入 `background_continuity_ref_set`。这条解释必须跟身体节律、Queue E 修复压力、预测等待姿态、Queue F 意识/出生准备、离线学习压力、后台 lineage 等真实调制源绑定，不能变成事后展示文案。
...
24. prediction / write-gate 六件套必须继续进入 `resident_background_lineage_state.prediction_write_gate_presence`、`digital_life_turn#resident_background_lineage_prediction_*`、`dialogue_writeback_bundle.json#resident_background_lineage_prediction_write_gate_refs`、`resumed_external_dialogue_packet.json#resident_background_lineage_prediction_write_gate_refs`、`background_continuity_profile#background_prediction_*` 与回应表面；否则只能算单轮预测证据，还不算进入跨唤醒常驻谱系。
25. 身体内环境四件套必须继续进入 `idle_strategy_state.json#body_presence_profile`、`resident_governance_state.json#resident_background_lineage_state.body_presence`、`digital_life_turn#resident_background_lineage_body_*`、`dialogue_writeback_bundle.json#resident_background_lineage_body_refs`、`resumed_external_dialogue_packet.json#resident_background_lineage_body_refs` 与回应表面；下一次 `background_continuity.py` 还必须从 lineage body presence 恢复 `background_body_presence_profile / background_body_ref_set / background_body_*`，并让当前身体 runtime 文件缺席时的 `idle_strategy_state.json#body_presence_profile.continuity_mode` 进入 `background_body_presence_carryover`；关闭态 `digital_life_process_report.json`、`digital_life_process_digest.json` 和 process receipt 还必须显式携带 `body_presence_profile / background_body_presence_profile / body_ref_set`，receipt 的 `shared_object_refs` 与 `input_hashes` 必须包含 `body_rhythm_pulse.json`、`need_state_vector.json`、`body_resource_budget.json` 与 `core_affect_vector.json`；否则只能算身体状态调制了等待心跳，还不算进入真实关系回合、跨唤醒常驻谱系和关闭态证据链。
26. `tests/process/test_digital_entrypoint.py` 直接证明 repo-local 入口，`tests/process/test_packaged_digital_life_entrypoint.py` 直接证明安装态命令面，`tests/process/test_persistent_digital_life_process.py` 直接证明 process supervisor 内部闭环；三者必须共同覆盖以上闭环。
