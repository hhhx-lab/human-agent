...
最新继续补强：Queue F 的 `identity_consciousness_birth_presence` 现在必须能从关闭态恢复为下一拍等待治理输入。`background_continuity.py` 需要从 nested presence、resident governance state/snapshot/report 与 persistent process report 中恢复 workspace、broadcast、metacognition、consciousness probe、birth readiness rollup/stage gate、reportability flags、birth readiness decision、next required command 与 refs，并同时写入 flat 字段和 `background_*` 字段；`idle_strategy.py` 在当前 Queue F 文件缺席时必须消费这些 background 字段，维持 `consciousness_reportable_waiting`、`birth_open_waiting` 或 `birth_blocked_waiting`，并继续调制 governance attention、cadence、heartbeat interval 与 next idle action；`heartbeat.py` 必须把 `background_identity_consciousness_birth_refs` 与 `background_birth_readiness_*` 写回 terminal loop state。这样 Queue F 不只在同一次等待态和真实回合里存在，也能跨关闭终端、重启和下一次唤醒保持出生准备姿态。
...
Queue F 第四轮不是“真实回合里能看到 Queue F presence”就算完成，而是必须跨关闭态恢复并继续调制等待治理：

1. `background_continuity.py` 已能从 nested `identity_consciousness_birth_presence` 与关闭态 report artifacts 恢复 `background_identity_consciousness_birth_presence`。
2. `background_continuity_profile` 同时写出 flat Queue F refs 与 `background_*` Queue F refs，并把 `background_identity_consciousness_birth_refs` 合并进 `background_continuity_ref_set`。
3. `idle_strategy.py` 在当前 workspace / broadcast / metacognition / consciousness probe / birth readiness 文件缺席时，仍能从 background profile 恢复 Queue F waiting posture。
4. `birth_open_waiting` 或 `birth_blocked_waiting` 必须继续影响 `governance_attention_target`、`governance_cadence_profile`、`heartbeat_interval_ms` 与 `next_idle_action`。
5. `heartbeat.py` 必须把 `background_identity_consciousness_birth_refs`、`background_consciousness_*` 与 `background_birth_readiness_*` 写回 terminal loop state，使下一轮真实关系回合还能继续构建 `identity_consciousness_birth_presence`。
