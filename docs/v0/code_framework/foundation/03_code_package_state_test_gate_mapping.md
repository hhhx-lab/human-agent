...
| `life_v0/process_supervisor/` | 最小常驻生命进程、heartbeat、incident/relaunch recovery | `digital_life_process_supervisor_engineering_contract.md` | `-（heartbeat.py / continuity_writeback.py / turn_io.py / dialogue_events.py / response_surface.py / incident_recovery.py / relaunch_recovery.py / process_report.py / idle_strategy.py / resident_supervision.py / idle_refresh_loop.py / live_turn_cycle.py / process_session_loop.py / persistent_process.py / resident_turn_writeback.py / process_closeout.py 已落；当前前沿转向后台 resident governance 与更高频 heartbeat 节律）` | `digital_life_waiting_heartbeat.json`、`digital_life_process_report.json`、`digital_life_process_digest.json`、incident/recovery reports | `tests/process/test_persistent_digital_life_process.py` | `waiting_heartbeat_gate`、`idle_continuity_gate`、`relaunch_recovery_gate` |
