...
如果已经确定当前要改哪个包，直接跳到对应行，不必整份读完。每一行都固定字段：
...
| `life_v0/process_supervisor/` | 最小常驻生命进程、heartbeat、incident/relaunch recovery、resident lifecycle、自主活动 | `digital_life_process_supervisor_engineering_contract.md` | `-（heartbeat.py / continuity_writeback.py / turn_io.py / dialogue_events.py / response_surface.py / incident_recovery.py / relaunch_recovery.py / process_report.py / idle_strategy.py / resident_supervision.py / idle_refresh_loop.py / live_turn_cycle.py / process_session_loop.py / persistent_process.py / resident_turn_writeback.py / process_closeout.py / resident_lifecycle.py / resident_autonomous_activity.py 已落；当前前沿转向 repo-local 与安装态共享 resident lifecycle 后的长时标治理）` | `digital_life_waiting_heartbeat.json`、`runtime/state/terminal/idle_strategy_state.json`、`runtime/state/terminal/resident_governance_state.json`、`runtime/state/terminal/resident_lifecycle_state.json`、`runtime/state/terminal/resident_relation_queue_state.json`、`runtime/state/terminal/resident_autonomous_activity_state.json`、`runtime/state/terminal/persistent_process_state.json`、`runtime/state/terminal/resident_governance_snapshot.json`、`digital_life_process_report.json`、`digital_life_process_digest.json`、incident/recovery reports | `tests/process/test_persistent_digital_life_process.py`、`tests/process/test_packaged_digital_life_entrypoint.py` | `waiting_heartbeat_gate`、`idle_continuity_gate`、`relaunch_recovery_gate` |
...
| `life_v0/digital_entry.py`、`digital` | repo-local `./digital life` 与安装后 `digital life` 共享入口与常驻过程启动 | `digital_life_process_supervisor_engineering_contract.md`、`digital_life_shell_command_engineering_contract.md` | `bootstrap_entry.py`、`stdin_loop.py`、`resident_lifecycle.py` | process boot receipts、process digests、resident lifecycle state | `tests/process/test_digital_entrypoint.py`、`tests/process/test_packaged_digital_life_entrypoint.py` | `digital_entry_gate` |
