...
| `process_supervisor/` | 最小常驻生命进程、heartbeat、idle continuity、incident/relaunch recovery | `real + thicken` | `heartbeat.py`、`continuity_writeback.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`incident_recovery.py`、`relaunch_recovery.py`、`idle_strategy.py`、`resident_supervision.py`、`idle_refresh_loop.py`、`live_turn_cycle.py`、`process_session_loop.py`、`persistent_process.py`、`process_report.py`、`resident_turn_writeback.py`、`process_closeout.py` | `-（resident turn continuity / resident supervision / idle refresh / live turn / session loop / closeout 已独立；当前前沿转向后台 resident governance 与更高频 heartbeat 节律）` | `20`、`44-46`、`82`、`86`、`89`、`90`、`96`、`101` | `IdleContinuityFrame`、`DialogueWritebackBundle`、`LifeContextFrame`、`RelationTurnFrame`、`ExpressionPlan`、`ReplayCueBundle`、`OfflineConsolidationFrame`、`GrowthPatchCandidate` | `digital_life_waiting_heartbeat.json`、`runtime/state/terminal/idle_strategy_state.json`、`runtime/state/terminal/persistent_process_state.json`、`digital_life_persistent_process_report.json`、`digital_life_process_report.json`、`digital_life_process_digest.json`、incident/recovery reports | `tests/process/test_persistent_digital_life_process.py`、`waiting_heartbeat_gate`、`idle_continuity_gate`、`relaunch_recovery_gate` |
