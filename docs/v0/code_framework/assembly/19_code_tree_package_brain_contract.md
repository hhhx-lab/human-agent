...
| `process_supervisor/` | 最小常驻生命进程、heartbeat、idle continuity、incident/relaunch recovery | `real + thicken` | `heartbeat.py`、`continuity_writeback.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`incident_recovery.py`、`relaunch_recovery.py`、`idle_strategy.py`、`idle_refresh_loop.py`、`persistent_process.py`、`process_report.py`、`resident_turn_writeback.py`、`process_closeout.py` | `-（resident turn continuity / idle refresh / closeout 已独立；下一前沿转向 resident_supervision.py）` | `20`、`44-46`、`82`、`86`、`89`、`90`、`96`、`101` | `IdleContinuityFrame`、`DialogueWritebackBundle`、`LifeContextFrame`、`RelationTurnFrame`、`ExpressionPlan`、`ReplayCueBundle`、`OfflineConsolidationFrame`、`GrowthPatchCandidate` | `digital_life_waiting_heartbeat.json`、`runtime/state/terminal/idle_strategy_state.json`、`runtime/state/terminal/persistent_process_state.json`、`digital_life_persistent_process_report.json`、`digital_life_process_report.json`、`digital_life_process_digest.json`、incident/recovery reports | `tests/process/test_persistent_digital_life_process.py`、`waiting_heartbeat_gate`、`idle_continuity_gate`、`relaunch_recovery_gate` |
...
3. `life_v0/process_supervisor/idle_refresh_loop.py` 已落第一轮，负责 idle heartbeat refresh、stdin probe 和 exit/next-turn 判定
4. 补厚 `heartbeat.py`
5. 补厚 `continuity_writeback.py`
6. 补厚 `dialogue_events.py`
7. 补厚 `response_surface.py`
8. 补厚 `process_report.py`
