...
| `process_supervisor/` | 最小常驻生命进程、heartbeat、idle continuity、incident/relaunch recovery、resident lifecycle、自主活动、模型外显表达证据 | `real + thicken` | `heartbeat.py`、`continuity_writeback.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`model_expression.py`、`incident_recovery.py`、`relaunch_recovery.py`、`idle_strategy.py`、`resident_supervision.py`、`idle_refresh_loop.py`、`live_turn_cycle.py`、`process_session_loop.py`、`persistent_process.py`、`governance_explanation.py`、`process_report.py`、`resident_turn_writeback.py`、`process_closeout.py`、`resident_lifecycle.py`、`resident_autonomous_activity.py` | `-（resident turn continuity / resident supervision / idle refresh / live turn / session loop / closeout / resident lifecycle / autonomous activity 已独立；当前前沿新增并加厚模型外显表达末端器官和 post-expression gate，并继续转向更长时标治理）` | `20`、`44-46`、`82`、`86`、`89`、`90`、`96`、`101` | `IdleContinuityFrame`、`DialogueWritebackBundle`、`LifeContextFrame`、`RelationTurnFrame`、`ExpressionPlan`、`LiveLanguageTurnState`、`PredictionWorkspaceFrame`、`ConsciousWorkspaceFrame`、`ReplayCueBundle`、`OfflineConsolidationFrame`、`GrowthPatchCandidate`、`ModelExpressionState` | `digital_life_waiting_heartbeat.json`、`runtime/state/terminal/idle_strategy_state.json`、`runtime/state/terminal/resident_governance_state.json`、`runtime/state/terminal/resident_lifecycle_state.json`、`runtime/state/terminal/resident_relation_queue_state.json`、`runtime/state/terminal/resident_autonomous_activity_state.json`、`runtime/state/language/model_expression_state.json`、`runtime/state/terminal/persistent_process_state.json`、`runtime/state/terminal/resident_governance_snapshot.json`、`digital_life_model_expression_report.json`、`digital_life_persistent_process_report.json`、`digital_life_resident_governance_report.json`、`digital_life_resident_governance_explanation.json`、`digital_life_process_report.json`、`digital_life_process_digest.json`、incident/recovery reports | `tests/process/test_model_expression.py`、`tests/process/test_persistent_digital_life_process.py`、`waiting_heartbeat_gate`、`idle_continuity_gate`、`relaunch_recovery_gate` |
...
1. `life_v0/process_supervisor/idle_strategy.py` 已落第一轮，继续补厚节律与 idle probe 治理，并把 `offline_pressure_level` / `relaunch_caution_level` / `next_idle_action` / `waiting_mode` 贯通到 heartbeat 与 closeout 证据
2. `life_v0/process_supervisor/persistent_process.py` 已落第一轮，继续补厚 resident supervision 关闭态治理快照与常驻治理报告
3. `life_v0/process_supervisor/resident_supervision.py` 已落第一轮，负责 restore 后的状态装载、relaunch normalization 和第一拍 waiting heartbeat 进入
4. `life_v0/process_supervisor/idle_refresh_loop.py` 已落第一轮，负责 idle heartbeat refresh、stdin probe 和 exit/next-turn 判定
5. `heartbeat.py` 已开始把 idle strategy 真实投射为等待轮询节律与 waiting heartbeat 治理字段
6. 补厚 `continuity_writeback.py`
7. 补厚 `dialogue_events.py`
8. 补厚 `response_surface.py`
9. `process_report.py` 已开始把 waiting governance 字段纳入主进程 report / receipt，后续继续补厚 digest 与 stage explain 消费面
