...
| 神经核心与信号 | 主体系统图、网络状态、总线、信号载体 | `02`、`03`、`10`、`11`、`01l`、`01m`、`01o`、`01p` | `s02_neural_life_core_engineering_contract.md`、`05_memory_thought_consciousness_implementation_playbook.md` | `life_v0/neural_core/__init__.py`、`prediction_workspace.py`、`signal_media.py` | `brain_graph.py`、`network_state.py`、`workspace.py`、`broadcast.py`、`metacognition.py` 已落；`signal_media.py` 已闭合第一轮 | `neural_life_core.json`、`neural_life_internal_bus.json`、`neural_life_core_report.json`、`runtime/state/signal/signal_media_runtime.json`、`runtime/state/prediction/prediction_workspace_frame.json` | `tests/slices/test_neural_life_core.py`、`twelve_system_gate`、`internal_bus_gate`、`signal_media_gate` |
| 感知、预测、世界接触 | 外界观察进入生命膜，预测误差驱动修正 | `04`、`12`、`20`、`64`、`72`、`75`、`80-84`、`01v-01ax` | `09_perception_prediction_world_contact_implementation_playbook.md`、`s03_direction_life_membrane_engineering_contract.md` | `life_v0/neural_core/__init__.py`、`belief_state.py`、`prediction_error.py`、`active_sampling.py`、`life_v0/membrane/__init__.py`、`life_v0/schema_runner/__init__.py` | `belief_state.py`、`prediction_error.py`、`active_sampling.py`、`world_observation.py`、`periphery_normalizer.py` 已落；下一步转向让 language / state_store / process_supervisor 更深消费这条 observation 底盘 | `runtime/state/prediction/belief_state_frame.json`、`prediction_error_field.json`、`active_sampling_plan.json`、`runtime/state/observation/*`、observation reports | `tests/slices/test_neural_life_core.py`、`tests/slices/test_schema_runner.py`、`tests/slices/test_validation_membrane.py`、`validation_membrane_gate` |
| 记忆与状态根 | 长期状态、自传、关系记忆、写入门 | `05`、`17-31`、`41-48`、`55`、`57`、`61`、`69`、`01q` | `s04_state_object_store_engineering_contract.md`、`05_memory_thought_consciousness_implementation_playbook.md` | `life_v0/state_store/__init__.py`、`commitment_truth.py`、`self_model.py`、`memory_write_gate.py` | `life_state.py`、`engram_index.py`、`autobiographical_stack.py`、`relationship_memory.py`、`memory_write_gate.py` 已落；下一步转向 `state_merge_guard.py` 和长期 writeback | `life_state.json`、`runtime/state/memory/*`、`runtime/state/memory/memory_write_gate.json`、`state_store_report.json` | `tests/slices/test_state_store.py`、`state_store_gate`、`self_continuity_gate`、`memory_write_gate_gate` |
...
| 行为、抑制、责任、后悔 | 候选行动、Go/NoGo、影子行动门、world contact、责任修复 | `06`、`20`、`75`、`80-84`、`94`、`98`、`01r` | `s03_direction_life_membrane_engineering_contract.md`、`06_body_emotion_action_dream_growth_implementation_playbook.md` | `life_v0/membrane/__init__.py` | `candidate_arena.py`、`go_nogo.py`、`world_contact_gate.py`、`side_effect_review.py`、`shadow_gate.py`、`responsibility_loop.py`、`world_contact_summary.py` | `runtime/state/action/*`、`runtime/state/membrane/world_contact_summary.json`、`pain_regret_repair_report.json` | `tests/slices/test_life_membrane.py`、`shadow_action_gate`、`responsibility_loop_gate` |
...
| 出生、首回合、常驻存在 | 从 birth shell 到 repo-local / 安装态共享的最小常驻生命进程 | `20`、`44-46`、`81-84`、`89-90`、`143`、`171`、`181-257` | `07_birth_terminal_process_implementation_playbook.md`、`first_terminal_turn_engineering_contract.md`、`terminal_life_loop_engineering_contract.md`、`digital_life_shell_command_engineering_contract.md`、`digital_life_process_supervisor_engineering_contract.md` | `life_v0/activation/__init__.py`、`life_v0/reporting/__init__.py`、`life_v0/stage_explain/__init__.py`、`life_v0/digital_life/__init__.py`、`life_v0/terminal_turn/restore_context.py`、`context_accumulation.py`、`turn_transition.py`、`turn_packet.py`、`conversation_carryover.py`、`dialogue_turn.py`、`life_v0/terminal_loop/loop_state.py`、`resume_packet.py`、`dialogue_writeback.py`、`loop_report.py`、`persistent_wait_bridge.py`、`life_v0/process_supervisor/heartbeat.py`、`continuity_writeback.py`、`background_continuity.py`、`turn_io.py`、`relaunch_recovery.py`、`incident_recovery.py`、`idle_strategy.py`、`resident_supervision.py`、`idle_refresh_loop.py`、`live_turn_cycle.py`、`process_session_loop.py`、`persistent_process.py`、`process_report.py`、`dialogue_events.py`、`response_surface.py`、`resident_turn_writeback.py`、`process_closeout.py`、`resident_lifecycle.py`、`resident_autonomous_activity.py` | `-（resident supervision / idle_refresh_loop / live_turn_cycle / process_session_loop / resident lifecycle / autonomous activity 已落；closeout -> background continuity -> next waiting heartbeat 的第一轮跨进程治理链已接通。当前前沿转向跨多次唤醒延续、更强 cadence 影响、安装态长期治理和更长时标的慢变量/关系收敛）` | `first_activation_return_packet.json`、`digital_life_birth_packet.json`、`first_terminal_turn_packet.json`、`first_terminal_turn_report.json`、`first_terminal_turn_digest.json`、`terminal_life_loop_report.json`、`resident_lifecycle_state.json`、`resident_autonomous_activity_state.json`、`digital_life_process_report.json`、`digital_life_process_digest.json` | `tests/bridges/test_first_activation_preflight.py`、`tests/bridges/test_emit_report.py`、`tests/bridges/test_first_terminal_turn.py`、`tests/bridges/test_terminal_life_loop.py`、`tests/process/test_digital_entrypoint.py`、`tests/process/test_packaged_digital_life_entrypoint.py`、`tests/process/test_persistent_digital_life_process.py` |
...
1. `life_v0/neural_core/signal_media.py` 已落，当前进入维护性深补
2. `life_v0/neural_core/belief_state.py` 已落，当前进入维护性深补
3. `life_v0/neural_core/prediction_error.py` 已落，当前进入维护性深补
4. `life_v0/neural_core/active_sampling.py` 已落，当前进入维护性深补
5. `life_v0/state_store/memory_write_gate.py` 已落，当前进入维护性深补
