...
| `life_v0/cli.py`、`life_v0/digital_entry.py`、`life_v0/runtime_config.py`、`digital` | `20`、`89`、`90`、`258` | repo-local 与安装态生命唤醒入口、命令面路由、resident lifecycle 命令面、统一运行配置读取 | activation / shell / process supervisors | CLI return codes、terminal outputs、entrypoint reports、resident lifecycle state、runtime config snapshot | `tests/process/test_digital_entrypoint.py`、`tests/process/test_packaged_digital_life_entrypoint.py` | 已落 repo-local 入口与项目级可安装 `digital life`；当前已补统一 `.env` / `DIGITAL_LIFE_ENV_FILE` 读取与去敏配置快照，后续进入 OS 级长期驻留前，继续守住 `.env` 配置、包边界和 resident lifecycle 证据链 |
