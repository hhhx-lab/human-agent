...
| `life_v0/process_supervisor/` | `20`、`44-46`、`81-84`、`86`、`89`、`90`、`96`、`101`、`181-257` | waiting heartbeat、resident supervision、resident lifecycle、autonomous activity、live turn dispatch、incident/relaunch recovery、session loop、persistent closeout、resident governance closeout、模型外显表达证据 | `LifeContextFrame`、`RelationTurnFrame`、`ExpressionPlan`、`ReplayCueBundle`、`OfflineConsolidationFrame`、`GrowthPatchCandidateQueue`、`relationship_timeline.json`、`commitment_expression_plan.json`、`apology_repair_language_trace.json`、Queue A 五件套、预测/意识工作区摘要、post-expression gate、`.env` 运行配置快照 | `IdleContinuityFrame`、`digital_life_waiting_heartbeat.json`、`runtime/state/terminal/idle_strategy_state.json`、`runtime/state/terminal/terminal_life_loop_state.json`、`runtime/state/terminal/resident_governance_state.json`、`runtime/state/terminal/resident_lifecycle_state.json`、`runtime/state/terminal/resident_relation_queue_state.json`、`runtime/state/terminal/resident_autonomous_activity_state.json`、`runtime/state/language/model_expression_state.json`、`runtime/state/terminal/persistent_process_state.json`、`runtime/state/terminal/resident_governance_snapshot.json`、`digital_life_model_expression_report.json`、`digital_life_persistent_process_report.json`、`digital_life_resident_governance_report.json`、`digital_life_process_report.json`、`digital_life_process_digest.json`、incident/recovery/relaunch reports | `tests/process/test_model_expression.py`、`tests/process/test_persistent_digital_life_process.py`、`tests/process/test_packaged_digital_life_entrypoint.py`、`waiting_heartbeat_gate`、`idle_continuity_gate`、`dialogue_process_receipt_gate` | 已落最小常驻进程、resident governance 运行态/关闭态、后台 resident lifecycle、自主活动 evidence 与 `.env` 驱动模型表达状态；`model_expression.py` 已从简单摘要加厚为消费本回合 percept / semantic / inner speech / expression monitor / prediction workspace 的外显表达末端器官，并新增 post-expression gate 防止模型输出把数字生命压回服务关系或擦掉生命证据 |
| `life_v0/cli.py`、`life_v0/digital_entry.py`、`life_v0/my_entry.py`、`life_v0/digital_life_identity.py`、`life_v0/runtime_config.py`、`digital`、`my` | `20`、`89`、`90`、`258` | repo-local 与安装态生命唤醒入口、命令面路由、首次命名锁、resident lifecycle 命令面、统一运行配置读取 | activation / shell / process supervisors / life name registry | CLI return codes、terminal outputs、entrypoint reports、resident lifecycle state、runtime config snapshot、`runtime/state/identity/life_name_registry.json` | `tests/process/test_digital_entrypoint.py`、`tests/process/test_packaged_digital_life_entrypoint.py`、`tests/process/test_my_digital_life_entrypoint.py` | 已落 repo-local 入口、项目级可安装 `digital life` 和推荐入口 `my digital life`；当前已补统一 `.env` / `DIGITAL_LIFE_ENV_FILE` 读取、去敏配置快照、首次命名登记与后续名字校验，后续进入 OS 级长期驻留前，继续守住 `.env` 配置、包边界、名字身份锚和 resident lifecycle 证据链 |
