...
| 包 | 脑科学/理论锚点 | 主职责 | 必入对象 | 必出对象 / runtime | 主测试 / gate | 当前 / 下一步 |
|---|---|---|---|---|---|---|
| `life_v0/membrane/` | `06`、`20`、`75`、`80-84`、`94`、`98`、`01r` | 候选行动、go/no-go、责任回路、side effect、world contact gate | `PredictionWorkspaceFrame`、`ExpressionPlan`、`NeedStateVector`、relationship pressure | `ActionCandidateSet`、`ActionIntentQueue`、`ObservationTruthGate`、`ConfirmationBinding`、`ResponsibilityLoopState`、`runtime/state/membrane/*`、`runtime/state/action/*` | `tests/slices/test_life_membrane.py`、`tests/slices/test_shadow_gate.py` | 第二波已落；继续补 `world_contact_summary.py` 与更厚的责任/确认回链 |
| `life_v0/validators/` | `29-36`、`49-84`、`102-118` | observation truth、boundary audit、validator membrane | action candidate、runtime observation、shell/world contact results | `WorldContactValidation`、`PredictionTraceValidation`、`runtime/state/validation/*`、validation reports | `tests/slices/test_validation_membrane.py` | 第二波已落；继续补 validator rollup 与 package-local gate 收口 |
| `life_v0/schema_runner/` | `33-36`、`49-60`、`102-180`、`01v-01ax` | consistency、counterfactual、cross-file logic、evidence rank、runner manifest | observation truth、responsibility loop、archive receipts | `runtime/state/schema_runner/*`、schema runner reports | `tests/slices/test_schema_runner.py`、`tests/slices/test_evidence_ranker.py` | 第一轮已落；现已显式消费 `ActionIntentQueue` / `ConfirmationBinding` / validator states，下一步补 stronger run manifest / cross-file closure and package-local gates |
