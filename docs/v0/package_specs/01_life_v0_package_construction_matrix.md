...
| `life_v0/cli.py`、`life_v0/digital_entry.py`、`digital` | `20`、`89`、`90`、`258` | repo-local 与安装态生命唤醒入口、命令面路由、resident lifecycle 命令面 | activation / shell / process supervisors | CLI return codes、terminal outputs、entrypoint reports、resident lifecycle state | `tests/process/test_digital_entrypoint.py`、`tests/process/test_packaged_digital_life_entrypoint.py` | 已落 repo-local 入口与项目级可安装 `digital life`；后续进入 OS 级长期驻留前，先守住 `.env` 配置、包边界和 resident lifecycle 证据链 |
