...
| `life_v0/terminal_loop/` | `20`、`86`、`89`、`90`、`96`、`101` | resumed loop、dialogue writeback、waiting-state handoff、loop report | `RelationTurnFrame`、`ExpressionPlan`、terminal state | `DialogueWritebackBundle`、`terminal_life_loop_state.json`、`terminal_life_loop_report.json` | `tests/bridges/test_terminal_life_loop.py`、`dialogue_writeback_bundle_gate` | 已落第一轮；继续补长期 relation continuity 与 bundle 强度 |

## G. 生命膜、验证与逻辑包

| 包 | 脑科学/理论锚点 | 主职责 | 必入对象 | 必出对象 / runtime | 主测试 / gate | 当前 / 下一步 |
|---|---|---|---|---|---|---|
| `life_v0/membrane/` | `06`、`20`、`75`、`80-84`、`94`、`98`、`01r` | 候选行动、go/no-go、责任回路、side effect、world contact gate | `PredictionWorkspaceFrame`、`ExpressionPlan`、`NeedStateVector`、relationship pressure | `ActionCandidateSet`、`ResponsibilityLoopState`、`runtime/state/membrane/*`、`runtime/state/action/*` | `tests/slices/test_life_membrane.py`、`tests/slices/test_shadow_gate.py` | 第一轮已落；继续补 `action_intent_bridge.py`、`observation_truth_gate.py`、`confirmation_binding.py` |
| `life_v0/validators/` | `29-36`、`49-84`、`102-118` | observation truth、boundary audit、validator membrane | action candidate、runtime observation、shell/world contact results | `runtime/state/validation/*`、validation reports | `tests/slices/test_validation_membrane.py` | 已落第一轮；继续补 world contact validator、prediction trace validator |
| `life_v0/schema_runner/` | `33-36`、`49-60`、`102-180`、`01v-01ax` | consistency、counterfactual、cross-file logic、evidence rank、runner manifest | observation truth、responsibility loop、archive receipts | `runtime/state/schema_runner/*`、schema runner reports | `tests/slices/test_schema_runner.py`、`tests/slices/test_evidence_ranker.py` | 第一轮已落；继续补 stronger run manifest / cross-file closure and package-local gates |
...
| `life_v0/process_supervisor/` | `20`、`44-46`、`81-84`、`86`、`89`、`90`、`96`、`101`、`181-257` | waiting heartbeat、resident supervision、live turn dispatch、incident/relaunch recovery、session loop、persistent closeout | `LifeContextFrame`、`RelationTurnFrame`、`ExpressionPlan`、`ReplayCueBundle`、`OfflineConsolidationFrame`、`GrowthPatchCandidateQueue` | `IdleContinuityFrame`、`digital_life_waiting_heartbeat.json`、`digital_life_process_report.json`、incident/recovery/relaunch reports | `tests/process/test_persistent_digital_life_process.py`、`waiting_heartbeat_gate`、`idle_continuity_gate` | 已落最小常驻进程；下一前沿转向高频 heartbeat、后台 resident governance、长期 waiting continuity 补厚 |
