...
32. `runtime/state/terminal/idle_strategy_state.json#cross_wake_trait_convergence_profile`
...
runtime/reports/latest/digital_life_resident_governance_explanation.json
  ...
  -> digital_life_turn cross_wake_trait_convergence_*
  -> dialogue_writeback_bundle.resident_background_lineage_refs
  -> dialogue_writeback_bundle.cross_wake_trait_convergence_refs
  -> resumed_external_dialogue_packet resident_background_lineage_trait_convergence_refs / cross_wake_trait_convergence_refs
  -> response_surface
```

这条链的关键点是：`trait_convergence_presence` 不再只是关系阶段和自我慢变量在后台治理里的摘要。`background_lineage_state.py` 现在会把人格慢变量历史焦点、稳定/不稳定名单、收敛评分、history profile、trait drift monitor ref 和 `trait_convergence_evidence_refs` 压进同一个 presence。证据 refs 至少要能吸收 resident governance state、governance explanation、trait drift monitor、background convergence summary 与 background convergence history 中已经存在的路径。`trait_convergence_signals.py` 则把这些来源进一步压成统一的 `cross_wake_trait_convergence_profile_v0`，让后续器官不需要各自重算 focus、pressure、稳定名单、不稳定名单、score 和 refs。

这条链当前又先进入关闭态证据链，再继续进入下一轮真实回合：`persistent_process.py` 与 `process_report.py` 会把 `cross_wake_trait_convergence_profile_v0` 写入 resident governance state/snapshot/report、persistent process report、process report、process digest 与 process receipt shared refs，证明画像不只存在于下一轮语言表面。随后 `dialogue_events.py` 会把 `resident_background_lineage_state_v0.trait_convergence_presence` 展开成 `resident_background_lineage_trait_convergence_history_focus`、`resident_background_lineage_trait_convergence_unstable_names`、`resident_background_lineage_trait_convergence_stable_names`、`resident_background_lineage_trait_convergence_score`、`resident_background_lineage_trait_drift_monitor_ref`、`resident_background_lineage_trait_convergence_history_profile` 与 `resident_background_lineage_trait_convergence_refs`，并把 trait refs 合并进 `resident_background_lineage_evidence_refs`；同时它会把 `cross_wake_trait_convergence_profile_v0` 作为同一事件的可复用画像字段写入 `digital_life_turn`。`resident_turn_writeback.py` 会让 lineage refs 跟随 `dialogue_writeback_bundle.resident_background_lineage_refs` 与 `resumed_external_dialogue_packet.json`，并把 `cross_wake_trait_convergence_refs` 放进专用写回槽；`response_surface.py` 会表达后台人格慢变量焦点、仍需收敛/已稳定名单、人格收敛评分、人格漂移监控在场、跨唤醒人格收敛画像、压力、评分和去重后的证据数量。
...
14. 下一次 `background_continuity.py` / `idle_strategy.py` / `heartbeat.py` 必须把这份关闭态 monitor ref 恢复成 `background_trait_drift_monitor_ref`；如果跨唤醒 history 已经带有 `trait_drift_update_mode_summary`、重校准名单或稳定名单，还必须恢复为 `background_trait_drift_update_mode_summary / recalibration_names / stabilized_names`，并继续写回 `idle_strategy_state.json`、`digital_life_waiting_heartbeat.json`、`idle_continuity_frame.json`、`resident_governance_state.json`、`terminal_life_loop_state.json` 与 `idle_heartbeat_trace.jsonl`
