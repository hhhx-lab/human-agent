...
| 内部证据回应 | `02-13`、`18-23`、`37-40`、`91-101`、`181-257` | `Packet D`、`Queue B`、`Queue D/E/F` 合同 | `response_surface.py#compose_life_response` | state/report/test 中的生命信号、lineage refs 与审计字段 |
| 外显 spoken response | `86` 的表达监控、`95-96` 的时间线、`94` 的痛苦/后悔/修复 | `Packet D waiting / response surface` | `response_surface.py#compose_life_spoken_response` | 终端输出受责任、梦境、出生修复、写门约束、身体情绪等信号调制，且不退回机械长清单或固定机制段落 |
