...
| `ResponsibilityLoopState` | `membrane/` | `responsibility_loop.py` | `language/commitment_repair.py`、`state_store/commitment_truth.py`、`state_store/relationship_memory.py`、`life_targets/`、`validators/` | `runtime/state/action/responsibility_loop_state.json` | `tests/slices/test_life_membrane.py`、`tests/slices/test_schema_runner.py` | 加深到 apology / repair time course 与 state writeback |
| `ActionCandidateSet` | `membrane/` | `candidate_arena.py` | `go_nogo.py`、`world_contact_gate.py`、`side_effect_review.py` | `runtime/state/action/action_candidate_set.json` | `tests/slices/test_life_membrane.py` | 与 `cross_file_logic.py`、`evidence_ranker.py`、`counterfactual_eval.py` 同步 |
