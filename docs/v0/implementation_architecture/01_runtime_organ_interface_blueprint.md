...
| 当前文件 | `heartbeat.py`、`continuity_writeback.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`incident_recovery.py`、`relaunch_recovery.py`、`resident_turn_writeback.py`、`persistent_process.py`、`process_report.py`、`process_closeout.py` |
...
| 下一步 | `idle_strategy.py`、`persistent_process.py`、`resident_turn_writeback.py` 与 `process_closeout.py` 已把 waiting / live turn / bundle writeback / closeout 接进同一条常驻连续体；下一步继续拆出更厚的 `idle_refresh_loop.py` |
