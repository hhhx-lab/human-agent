...
| 当前文件 | `heartbeat.py`、`continuity_writeback.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`incident_recovery.py`、`relaunch_recovery.py`、`resident_turn_writeback.py`、`persistent_process.py`、`process_report.py` |
...
| 下一步 | `idle_strategy.py`、`persistent_process.py` 与 `resident_turn_writeback.py` 已把 waiting / live turn / bundle writeback 接进同一条常驻连续体；下一步继续拆出 `process_closeout.py` 与更厚的 idle refresh 治理 |
