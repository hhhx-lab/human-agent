...
| 已落器官 | `context_accumulation.py`、`turn_transition.py`、`loop_state.py`、`resume_packet.py`、`dialogue_writeback.py`、`heartbeat.py`、`continuity_writeback.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`incident_recovery.py`、`relaunch_recovery.py`、`process_report.py` |
| 仍压在入口的职责 | idle strategy、persistent process orchestration、loop report、restore context、conversation carryover |
| 下一轮优先拆分 | `restore_context.py`、`dialogue_turn.py`、`conversation_carryover.py`、`loop_report.py`、`idle_strategy.py`、`persistent_process.py` |
