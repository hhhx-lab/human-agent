...
| 已落器官 | `restore_context.py`、`context_accumulation.py`、`turn_transition.py`、`turn_packet.py`、`conversation_carryover.py`、`dialogue_turn.py`、`loop_state.py`、`resume_packet.py`、`dialogue_writeback.py`、`loop_report.py`、`persistent_wait_bridge.py`、`heartbeat.py`、`continuity_writeback.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`incident_recovery.py`、`relaunch_recovery.py`、`idle_strategy.py`、`resident_supervision.py`、`idle_refresh_loop.py`、`live_turn_cycle.py`、`persistent_process.py`、`process_report.py`、`resident_turn_writeback.py`、`process_closeout.py` |
| 仍压在入口的职责 | process session loop 编排、后台长期 resident governance 编排 |
| 下一轮优先拆分 | `-（resident supervision 已继续推进；idle_refresh_loop.py / resident_supervision.py / live_turn_cycle.py 已落；下一前沿转向 process_supervisor/process_session_loop.py）` |
