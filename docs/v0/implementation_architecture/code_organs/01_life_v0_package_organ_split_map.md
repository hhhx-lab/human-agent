...
| 已落器官 | `restore_context.py`、`context_accumulation.py`、`turn_transition.py`、`turn_packet.py`、`conversation_carryover.py`、`dialogue_turn.py`、`loop_state.py`、`resume_packet.py`、`dialogue_writeback.py`、`loop_report.py`、`persistent_wait_bridge.py`、`heartbeat.py`、`continuity_writeback.py`、`background_continuity.py`、`background_convergence.py`、`background_convergence_history.py`、`background_lineage_state.py`、`process_lease.py`、`resident_autonomous_activity.py`、`resident_governance_handoff.py`、`resident_lifecycle.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`trait_convergence_signals.py`、`incident_recovery.py`、`relaunch_recovery.py`、`idle_strategy.py`、`resident_supervision.py`、`idle_refresh_loop.py`、`live_turn_cycle.py`、`process_session_loop.py`、`persistent_process.py`、`governance_explanation.py`、`process_report.py`、`resident_turn_writeback.py`、`process_closeout.py` |
| 仍压在入口的职责 | 跨多次唤醒的后台 resident governance 演化，以及 waiting governance 字段到更上层运行时与长期解释面的后续消费 |
| 下一轮优先拆分 | `-（resident supervision / idle_refresh_loop / live_turn_cycle / process_session_loop 已落；background_continuity.py 也已把 closeout artifact 重装成下一次 waiting heartbeat 可消费的后台连续体 profile。当前前沿转向 idle_strategy.py / heartbeat.py / persistent_process.py / process_closeout.py 的跨多次唤醒治理补厚）` |
| runtime 产物 | `session_envelope.json`、`context_accumulation_window.json`、`turn_transition_trace.json`、`terminal_life_loop_state.json`、`digital_life_waiting_heartbeat.json`、`runtime/state/terminal/idle_strategy_state.json`、`runtime/state/terminal/resident_governance_state.json`、`runtime/state/terminal/resident_process_lease.json`、`runtime/state/terminal/resident_process_lease_history.jsonl`、`runtime/state/terminal/resident_process_lease_history_profile.json`、`runtime/state/terminal/resident_autonomous_activity_state.json`、`runtime/state/terminal/resident_lifecycle_state.json`、`runtime/state/terminal/background_convergence_summary.json`、`runtime/state/terminal/background_convergence_history.json`、`runtime/state/terminal/persistent_process_state.json`、`runtime/state/terminal/resident_governance_snapshot.json`、`digital_life_resident_governance_report.json`、`digital_life_resident_governance_explanation.json`、`digital_life_process_report.json` |
