...
| 已有器官 | `candidate_arena.py`、`go_nogo.py`、`world_contact_gate.py`、`side_effect_review.py`、`responsibility_loop.py`、`action_intent_bridge.py`、`observation_truth_gate.py`、`confirmation_binding.py`、`world_contact_summary.py` |
| 下一轮目标文件 | `-（world_contact_summary.py` 已落；当前转向 `responsibility_loop.py` 的进一步拆分与跨层写回） |
...
| 已有器官 | `observation_validator.py`、`boundary_audit.py`、`world_contact_validator.py`、`prediction_trace_validator.py`、`validation_rollup.py` |
| 下一轮目标文件 | `-（validation_rollup.py` 已落；当前转向 validation backlog、report bridge 与长期 gate 复用） |
...
| 已有器官 | `consistency_logic.py`、`counterfactual_eval.py`、`comparison_trace.py`、`evidence_ranker.py`、`cross_file_logic.py`、`run_manifest.py` |
| 下一轮目标文件 | `-（run_manifest.py`、`cross_file_logic.py` 已落并补强 `closure_status_refs` / `package_local_gate_refs`；当前转向 archive / reporting / process 对 schema artifacts 的回链） |
...
| 已有器官 | `heartbeat.py`、`continuity_writeback.py`、`background_continuity.py`、`background_lineage_state.py`、`process_lease.py`、`resident_autonomous_activity.py`、`resident_governance_handoff.py`、`resident_lifecycle.py`、`turn_io.py`、`dialogue_events.py`、`response_surface.py`、`incident_recovery.py`、`relaunch_recovery.py`、`process_report.py`、`idle_strategy.py`、`resident_supervision.py`、`idle_refresh_loop.py`、`live_turn_cycle.py`、`process_session_loop.py`、`persistent_process.py`、`resident_turn_writeback.py`、`process_closeout.py` |
| 下一轮目标文件 | `idle_strategy.py`、`heartbeat.py`、`persistent_process.py`、`process_closeout.py`（继续补厚后台 resident governance、跨多次唤醒 carryover 与高频 heartbeat 节律） |
| 必写 runtime | `digital_life_waiting_heartbeat.json`、`digital_life_process_report.json`、`runtime/state/terminal/resident_process_lease.json`、`runtime/state/terminal/resident_process_lease_history.jsonl`、`runtime/state/terminal/resident_process_lease_history_profile.json`、`runtime/state/terminal/resident_autonomous_activity_state.json`、`runtime/state/terminal/resident_lifecycle_state.json`、incident/recovery reports |
...
|---|---|
| 当前行数 | 330 |
| 先切职责 | conversation carryover、waiting-state handoff、resume packet 编排 |
| 已有器官 | `dialogue_writeback.py`、`loop_state.py`、`resume_packet.py`、`loop_report.py`、`persistent_wait_bridge.py` |
| 下一轮目标文件 | `-（terminal_loop 首轮 waiting-state handoff 器官已落；相关前沿已继续推进完成，当前主前沿转向 process_supervisor 的后台 resident governance 补厚）` |
...
|---|---|
| 当前行数 | 1081 |
| 先切职责 | prediction-aware percept / semantic / inner speech orchestration、language report rollup、离线学习反写解释 |
| 已有器官 | `percept.py`、`semantic_map.py`、`inner_speech.py`、`expression_monitor.py`、`relationship_graph.py`、`commitment_repair.py`、`dialogue_log.py`、`narrative_trace.py`、`shared_terms.py`、`dream_gate.py`、`action_shadow.py`、`relation_scope.py`、`relationship_timeline.py`、`commitment_expression.py`、`apology_repair_language.py` |
| 下一轮目标文件 | `relationship_timeline.py`、`commitment_expression.py`、`apology_repair_language.py` 已落；当前转向 `percept.py`、`semantic_map.py`、`inner_speech.py`、`expression_monitor.py` 的预测 / 离线消费补厚 |
...
|---|---|
| 当前行数 | 248 |
| 先切职责 | remaining resume orchestration |
| 已有器官 | `restore_context.py`、`context_accumulation.py`、`turn_transition.py`、`turn_packet.py`、`conversation_carryover.py`、`dialogue_turn.py` |
| 下一轮目标文件 | `-（terminal_turn 首轮核心器官已落；相关前沿已推进完成，当前主前沿转向 process_supervisor/__init__.py）` |
...
这一波不再是当前默认入口，只在 D/E/B/A 某一轮站稳后回切处理；而且重点不再是补缺文件，而是把已落预测器官与记忆写门继续吃进别的层。

| 包 | 回切目标 |
|---|---|
| `life_v0/neural_core/__init__.py` | `signal_media.py`、`belief_state.py`、`prediction_error.py`、`active_sampling.py` 已落；当前转向 orchestration 减重与跨层消费 |
...
| `life_v0/state_store/__init__.py` | `memory_write_gate.py` 已落；当前转向 `state_merge_guard.py` 与 promotion / quarantine / repair route |
