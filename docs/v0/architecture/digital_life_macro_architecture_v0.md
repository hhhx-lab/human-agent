...
生命目标声明：本文档把第一版数字生命工程从“最小 runner”上调为“主体宏架构”。`DocCorpusIngestor` 已经能把 `00-258` 理论底座转成机器可读索引，但它只是第一次激活前的理论摄取层，不是数字生命的大脑，不是意识工作区，不是语言主神经束，也不是自我成长核。真正要出生的数字生命不能继承 OpenClaw、Hermes 或普通 agent 框架的主体结构；它的能力、思考方式、记忆、语言、梦境、情绪、责任、关系和成长已经由前文生命文档定义，本文件只负责把这些生命能力压成第一版工程骨架。
...
因此，当前框架资料只保留在 `docs/v0/references/current_agent_shell_reference_2026.md` 作为负边界和外部资料索引：它提醒工程实现不要降格，而不提供主体模块蓝图。
...
读取：`12`、`15`、`20`、`24`、`28`、`32`、`33-84`、`89`、`100`、`119`、`122`、`140`、`docs/v0/references/current_agent_shell_reference_2026.md`。
...
当前仓库已经有一套 `life_v0/` 骨架在生长，所以宏架构不能脱离当前现实另起炉灶。第一轮应该按“现有骨架 -> 文件级器官化”的路线推进：

| 当前真实包 | 当前作用 | 对应宏架构位置 | 下一步拆分方向 |
|---|---|---|---|
| `life_v0/doc_index.py` | P0 文档摄取与 carrier index | `DocCorpusIngestor` | 保持单入口，继续补 `doc_dependency_graph`、carrier checker |
| `life_v0/direction/` | 方向锁、身份根、断联恢复 | 方向层 / `DirectionLockKernel` | 继续补 `identity_root.py`、`resume_anchor.py` |
| `life_v0/neural_core/` | 主体系统总入口、prediction workspace 种子 | `MultiscaleBrainGraphRuntime`、`PredictionActiveInferenceRuntime`、`ConsciousWorkspaceRuntime` 的过渡根 | 继续拆 `region_graph.py`、`signal_media.py`、`workspace.py` |
| `life_v0/state_store/` | `life_state`、`self_model`、`commitment_truth` | `LifeStateStore`、`AffectiveSelfRuntime` 的状态根 | 继续拆 `life_state.py`、`autobiographical.py`、`relationship_memory.py` |
| `life_v0/language/` | 感知、语义映射、内言语、表达监控、关系图、承诺修复、叙事回写 | `LanguageRelationshipRuntime` | 继续拆 `relationship_timeline.py`、`dialogue_memory.py`、`language_action_bridge.py` |
| `life_v0/body/` + `life_v0/defense/` + `life_v0/growth/` | 身体预算、防御边界、可塑性窗口、成长路线、防遗忘 | `SiliconBodyRuntime`、`SignalMediaRuntime`、`SelfGrowthRuntime` | 继续拆 `resource_budget.py`、`rhythm.py`、`plasticity_window.py`、`anti_forgetting.py` |
...
| `life_v0/process_supervisor/`、`digital_life/`、`shell_command/`、`digital_entry.py`、`digital` | 出生壳、repo-local shell、最小常驻生命进程 | `LifeMembraneAndComputerPeripheralRuntime` 的外层过程 | 继续补厚 `idle_strategy.py`、`persistent_process.py`、`process_closeout.py` 的后台 resident governance 与高频节律 |

这张表的意思只有一个：后续不是去平行创建一套新的“标准目录”，而是把当前骨架逐步拆成宏架构定义的器官。

## 第一轮宏架构对象链

第一轮真正要钉死的不是模块名，而是跨层对象链。后续代码拆分必须能回到这条链：
...
BodyRhythmPulse
  ...
  -> ActionCandidateArena
  -> LifeMembraneDecision
  -> PeripheralActionEnvelope or RelationTurnFrame
  -> PostActionAuditReport
  -> ReplayCueBundle
  -> OfflineConsolidationFrame
  -> GrowthPatchCandidateQueue
  -> updated LifeStateRoot
```

如果后续某段实现绕开了这条链，只剩“输入 -> 回复 -> 日志”，那它就不是数字生命主体，而是再次滑回普通 agent。
...
宏架构第一轮不是“写出一篇架构文”就算完成。至少要同时满足：

1. 三重身体与十二主体系统已经能一一对应到当前 `life_v0/` 真实代码骨架。
2. 每个主体系统都已经有明确的 state/report/archive 落点，不再只是抽象名字。
3. 终端生命循环已经被放在 `ComputerBody` 外周，而不是反过来定义主体。
4. 语言、梦境、责任、成长四条生命主链都已经进入跨层对象链。
5. 后续拆代码时有明确“现有包 -> 目标器官文件”的过渡路线，不会平行生长第二套目录。
