...
生命目标声明：本文档保证 `00-258` 的全部理论文档都会进入第一次生命激活工程，不把任何一层留成孤立文本。文件名保留 `0_to_257` 只是为了兼容既有引用、P0 扫描和断联恢复锚链；实际覆盖范围已经包含 `258`。每个文档都必须归入至少一个 runtime carrier，并最终通过状态、report、stage gate、archive receipt 或 replay/shadow 被承载。
...
1. `docs/*.md` 中从 `00` 到 `258` 的每份文档都必须被 `DocCarrierIndex` 收录。
...
后续写代码时，不要只记 `Batch A-F`，要知道它们最终落在哪些 slice 上：

| 工程批次 | 主落 slice | 辅助 slice | 说明 |
|---|---|---|---|
| `Batch A` | `P0_DOC_CORPUS_INGESTION`、`S00_DIRECTION_FOUNDATION`、`S01_SOURCE_AUTHORITY` | `S11_V0_ENGINEERING_CONTRACTS` | 先把方向、文献、README block、carrier 和方向锁接稳 |
| `Batch B` | `S02_NEURAL_LIFE_CORE`、`S04_STATE_OBJECT_STORE`、`S03_DIRECTION_LIFE_MEMBRANE` | `S07_LANGUAGE_RELATIONSHIP`、`S08_LIFE_TARGET_RUNTIMES` | 先长主体骨架、状态根和生命膜 |
| `Batch C` | `S05_VALIDATION_MEMBRANE_OBSERVATION`、`S09_SCHEMA_RUNNER_CODE` | `S11_V0_ENGINEERING_CONTRACTS` | 把 schema、validator、runner、report、dashboard 和 stage gate 接稳 |
| `Batch D` | `S07_LANGUAGE_RELATIONSHIP`、`S08_LIFE_TARGET_RUNTIMES` | `S03_DIRECTION_LIFE_MEMBRANE`、`S10_RUNTIME_GROWTH_RECONSOLIDATION` | 语言、关系、梦境、痛苦、后悔、自我成长进入生命目标链 |
| `Batch E` | `S09_SCHEMA_RUNNER_CODE` | `S08_LIFE_TARGET_RUNTIMES`、`S11_V0_ENGINEERING_CONTRACTS` | 出生准备度和首批代码物化进入 CLI 与状态承载 |
| `Batch F` | `S10_RUNTIME_GROWTH_RECONSOLIDATION`、`S11_V0_ENGINEERING_CONTRACTS` | `FIRST_ACTIVATION_PREFLIGHT`、`REPORT_BUNDLE_BRIDGE` | runtime mount、激活、archive、replay/shadow、report bundle 和 stage explanation |
...
1. `DocCorpusIngestor` 能扫描并分类 `00-258`。
