...
| 神经核心与预测 | `life_v0/neural_core/` | `brain_graph.py`、`network_state.py`、`signal_media.py`、`belief_state.py`、`prediction_error.py`、`active_sampling.py`、`prediction_workspace.py`、`broadcast.py` | `runtime/state/neural_life_core/*`、`runtime/state/signal/*`、`runtime/state/prediction/*` | `tests/slices/test_neural_life_core.py` | 下一步转向 membrane / language / life_targets 对预测器官的深消费 |
| 状态根与记忆 | `life_v0/state_store/` | `life_state.py`、`engram_index.py`、`relationship_memory.py`、`commitment_truth.py`、`memory_write_gate.py` | `runtime/state/life_state.json`、`runtime/state/memory/*`、`runtime/state/self/*` | `tests/slices/test_state_store.py` | 下一步转向 `state_merge_guard.py` 与长期写回策略 |
