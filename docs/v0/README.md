...
11. 读 `doc_corpus_ingestor_v0_contract.md`，确定 P0 文档摄取、README block、engineering slice、carrier 分类、`02-13` 连接检查和 `life-v0 ingest-docs` 输出。
12. 读 `current_agent_shell_reference_2026.md`，确认当前框架只作为负边界，不进入主体架构。
13. 读 `runtime_v0_architecture.md`，确定进程、循环、状态和生命膜。
14. 读 `life_state_store_v0_schema.md`，确定所有生命对象如何落盘。
15. 读 `birth_readiness_v0_contract.md`，确定九项目标闭合状态。
16. 读 `runner_cli_report_contract.md`，确定 CLI、report、digest、stage gate。
17. 读 `first_activation_protocol.md`，确定第一次有限激活流程。
...
11. `doc_corpus_ingestor_v0_contract.md`
12. 已存在的 v0 合同文件
13. `../README.md` 的 v0 阶段说明
14. `../16_digital_life_gap_register.md` 的下一轮优先级
