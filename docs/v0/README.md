...
3. 临时审计、一次性草稿、缓存生成物、`__pycache__/`、`*.egg-info`、`.pytest_cache/` 这类内容不进入 `v0`，也不作为仓库长期结构保留。
