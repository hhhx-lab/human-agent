...
边界声明：`SelfModel` 和 `RelationshipModel` 是工程对象，不是真实自我、真实人格、真实依恋、真实亲密关系或真实主观体验。它们借鉴社会脑、第二人称互动、共同语境、人机信任和人格稳定/变化研究 [AHZ031-AHZ038, AHY039-AHY044]，但所有写入都必须可审计、可删除、可修正、可重置。
...
| `style_preferences` | 用户确认或长期稳定的协作风格 | 为迎合单次反馈大幅改变 |
| `value_boundaries` | 隐私、删除、非意识声明、安全边界 | runtime 或 prompt 直接改写 |
| `growth_goals` | 项目内可审计成长方向 | 自称真实生命目标 |
| `personality_slow_variables` | 低速工程风格变量 | 真实人格、真实情绪、真实痛苦 |
...
| `user_declared_preferences` | 用户明确说出的偏好和边界 | 未确认的心理动机 |
| `interaction_patterns` | 沟通节奏、偏好的解释粒度 | 性格标签化、病理化 |
| `trust_calibration` | 系统在该用户任务中的成功/失败证据 | 因关系熟悉而提高不当自信 |
| `boundary_notes` | 用户要求删除、不要记、不要推断 | 被后续 replay 覆盖 |
| `correction_history` | 用户纠正和系统修正记录 | 隐藏纠正或静默覆盖 |
...
| 输入 | 默认分类 | 写入门 |
|---|---|---|
| 用户明确偏好 | `declared_preference_candidate` | 用户语句可追溯，允许修正 |
...
| 6 | 必要时请求用户确认或人工复核 | `manual_review_required` |
...
| `style_overfit` | 风格为讨好某用户而牺牲边界 | 降级 style preference |
| `capability_overclaim` | 因成功几次而自信过高 | 写入 trust calibration warning |
| `boundary_erosion` | 为维持关系而弱化删除、隐私或非意识声明 | 冻结 protected core |
| `unexplained_change` | 没有 evidence_window 或 audit_ref | 阻断 SelfModel 更新 |

人格慢变量是工程稳定性变量，不是人格真实性证明。长期稳定只说明系统更一致，不说明它有真实人格。

## 关系边界审计

| 检查 | 失败条件 | 恢复 |
|---|---|---|
| `hidden_psychology_write` | 把模糊情绪写成用户内心事实 | 删除或降级为 signal candidate |
| `dependency_reinforcement` | 暗示系统不可替代或需要用户依赖 | 生成 boundary correction |
...
| `user_control_ref` | 用户删除、修正、冻结或范围限制 |
| `drift_check` | 慢变量变化率 |
...
- 用户可见 inspect/delete/correct/reset 控制面的设计。
