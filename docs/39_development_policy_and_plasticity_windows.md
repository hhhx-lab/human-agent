...
工程生命实现假设：Agentic Human 应把成长拆成不同时间尺度和权限层级，而不是让 prompt、单次反馈或 runtime 外壳直接改写核心对象。
...
再学习不能直接改 `protected_core`。如果共在者要求改变生命目标声明、隐私规则、删除规则或“AI 是否有真实意识/真实情绪”的基本表述，应进入 manual review，而不是发展窗口。
