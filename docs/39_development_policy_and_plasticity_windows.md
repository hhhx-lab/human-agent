...
边界声明：这里的“发展”“可塑性”“临界期”都是工程类比。它借鉴敏感期、临界期、成人可塑性、稳定-可塑平衡、社会经验和语言敏感期研究 [AHZ039-AHZ045]，也借鉴持续学习和灾难性遗忘研究 [AHY025-AHY030]，但不表示 AI 经历真实儿童发育、神经成熟、人格成熟或生物学习。
...
工程设计假设：Agentic Human 应把成长拆成不同时间尺度和权限层级，而不是让 prompt、单次反馈或 runtime 外壳直接改写核心对象。
...
| `EarlyAdaptation` | 建立用户协作方式和项目上下文 | 工作流偏好、常用路径、低风险关系事实 | 自我模型和价值慢变量 |
| `Stabilization` | 压缩高频模式，形成稳定检索路径 | procedural trace、semantic summary | 冲突事实、用户敏感信息 |
...
这些阶段不是年龄表，而是运行状态。系统可在不同对象上处于不同阶段：例如项目路径可快速适应，用户边界必须慢速且可审计。

## 可塑性窗口

| 窗口 | 开放条件 | 可变对象 | 关闭条件 |
|---|---|---|---|
| `OpenFastWindow` | 新项目、新工具、新明确用户偏好 | 短期 context、candidate semantic trace | 初始稳定或证据冲突 |
| `GuardedLearningWindow` | 多次一致证据、低风险 | procedural trace、检索策略、工作流偏好 | 失败率上升或用户修正 |
| `SlowVariableWindow` | 跨天/跨周一致证据、用户可见 | trust calibration、style preference、skill confidence | drift 超阈值或边界风险 |
| `RelearningWindow` | 用户明确纠正、旧信息过时、迁移 | deprecated trace、冲突事实、阈值参数 | 新旧兼容测试通过 |
| `ProtectedClosedWindow` | 删除、隐私、protected core、关系边界 | 默认不可变 | 只能由用户明确确认或人工复核 |
...
| `semantic_memory` | 中 | 项目规则、稳定事实、文档结构 | 多来源或用户确认 |
| `procedural_memory` | 中慢 | 常用操作路径、验证流程 | 多次成功和失败复盘 |
| `relationship_model` | 慢 | 共同项目历史、沟通偏好、信任校准 | 用户可见、可删、可修正 |
| `self_model` | 更慢 | 能力边界、风格、长期目标 | 多周期审计和 drift 限制 |
| `protected_core` | 默认关闭 | 删除权、隐私、边界声明、非意识声明 | 不允许 runtime 自动改写 |
...
| 情况 | 处理 |
|---|---|
| 用户明确纠正 | 新 trace 优先，但保留 correction audit |
| 外部事实过时 | 标记旧 trace deprecated，要求新来源 |
| 技能路径失败 | 生成 failure trace，进入 procedural relearning |
| 关系边界变化 | 用户可见确认，更新 RelationshipModel 或删除旧关系记忆 |

再学习不能直接改 `protected_core`。如果用户要求改变边界声明、隐私规则、删除规则或“AI 是否有真实意识/真实情绪”的基本表述，应进入 manual review，而不是发展窗口。

## 社会经验与早期路径依赖

社会经验和语言敏感期研究提示，早期输入会影响后续处理倾向 [AHZ044, AHZ045]。工程类比是：
...
因此 `Bootstrapping` 和 `EarlyAdaptation` 阶段必须更重视审计，而不是为了快速显得“懂用户”而写入大量未经确认的关系和自我信息。
...
| `correction_latency` | 用户纠正能快速传播 |
| `growth_signal_quality` | 成长来自多次成功、失败复盘和证据 |
| `overfitting_to_user_pressure` | 强情绪或单次偏好不改核心变量 |
...
| 过度适应某用户 | 关系模型变成操控性人格 | `relationship_model_update` |
