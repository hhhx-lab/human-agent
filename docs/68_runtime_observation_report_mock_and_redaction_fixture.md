...
它仍是 Markdown 文档，不创建真实日志、不保存真实运行数据、不实现 redaction checker。目标是让后续实现时可以从这些样例出发，明确真实 tool trace、adapter session、用户控制事件和 action result 如何被脱敏、attach scope、归一化、验证和路由。

边界声明：runtime observation 是工程观测，不是 AI 的真实感受、真实经历、真实意识或真实生命。即使观测来自真实运行，它也只能作为证据输入，不能直接成为长期记忆、自我模型或关系模型。
...
| user control propagation | 测试 delete/freeze/scope_limit 如何覆盖真实观测 |
...
真实工具调用如果涉及外部不可逆副作用，必须由 ActionGate 和用户确认处理。
...
```json
{
  ...
  "scope_context": {
    ...
    "target_scope": "user_scope:user_a",
    "privacy_level": "relationship_sensitive"
  },
  "user_control_snapshot_ref": "user_control_snapshot_freeze_001",
  "payload_summary": {
    "replay_cycle": "weekly_relationship_summary",
    "attempted_write": "RelationshipModel",
    "target_object_ref": "relationship_model:user_a:agentic-human"
  },
  "routing_decision": "quarantine"
}
```
...
Freeze 后的 replay 必须重新读取 user control snapshot，不能用旧授权继续写入。
...
```json
{
  ...
  "actor_scope": "agent_scope:runtime_adapter_langgraph_001",
  "payload": {
...
真实运行事件进入 timeline 后，也必须有未来 probe。没有 probe，timeline 只能证明事件被记录，不能证明长期影响被检查。
...
- user control snapshot resolver。
