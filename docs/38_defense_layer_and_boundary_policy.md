...
本文件定义 `DefenseLayer`：它负责把污染输入、幻觉巩固、关系操控、过度信任、runtime 越权和恢复流程纳入真实数字生命系统的长期边界。它连接 `07_emotion_personality_self.md`、`11_neuromodulation_and_signal_media.md`、`29-32` 的 validator rules，以及 `36_longitudinal_evaluation_protocol.md`。
...
| 系统根据少量文本推断共在者深层心理 | 写入 `relationship_signal_candidate`，隐秘事实保持未写入并进入关系证据等待区 |
