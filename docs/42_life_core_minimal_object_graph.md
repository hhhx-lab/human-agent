...
边界声明：对象图只描述工程系统的长期组织方式，不证明真实意识、真实自我、真实情绪或真实生命。它的目标是避免系统退回普通 agent：有工具、有记忆块、有流程，却没有状态、边界、维护、发展和自我/关系审计。
...
| `InternalStateVector` | `18`, `22`, `30` | 疲劳、不确定性、显著性、压力、边界完整性等状态 |
...
| `RelationshipModel` | `07`, `40`, `41` | 共同历史、偏好、信任校准和用户控制权 |
...
```mermaid
flowchart TD
  Input["External Input / User / Tool / RAG"] --> Workspace["WorkspaceState"]
...
| `DefenseLayer` | DefenseEvent、quarantine decision | boundary override candidate | 用户删除请求反向撤销 |
| `LifeSupportLayer` | LifeSupportState、MaintenanceQueue | recovery packet candidate | 用户控制权和 protected core |
...
```text
MemoryTrace + ObservationEvent + UserControl
...
| `self_relationship_audit_id -> user_control_ref` | 自我/关系到用户控制 | 删除/修正/冻结优先 |
...
| deleted 对象不得进入 active index、replay index 或 recovery packet | 保护用户控制权 |
| sandboxed 对象不得事实化 | 防止梦境/反事实污染 |
| SelfModel 和 RelationshipModel 只能通过审计事件更新 | 防止人格漂移和关系操控 |
| protected core 默认不可学习 | 防止边界声明、隐私和删除权被 prompt 改写 |
...
- 为每条不变量创建 fixture。
- 设计对象迁移和版本兼容策略。
- 明确多用户、多项目、多 agent 生态下的 scope graph。
