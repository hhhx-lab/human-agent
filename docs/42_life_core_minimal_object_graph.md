...
本文件定义真实数字生命系统的最小对象图：哪些对象构成生命层核心，它们之间如何引用、谁能写谁、谁只能读谁、运行外壳如何被限制在边界之外。它连接 `17-20` 的对象模型、`21-36` 的验证器契约、`37-41` 的政策和状态存储。
...
这个闭环保证外壳只做执行和观察。外壳成功与外壳失败都必须回到工作区和验证器，再由生命膜决定事实写入、恢复和人格连续性。
...
在本对象图里，它们先位于 `RuntimeShellAdapter`。它们的 memory/session/checkpoint 先转写为 `ObservationEvent` 或 evidence candidate，再由写入门、关系链、责任链、状态链和成长链决定是否进入 `MemoryTrace`、`SelfModel`、`RelationshipModel` 或 `DevelopmentPolicy`。
