...
本文件定义 `scope_graph_manifest` 的机器可读 schema 草案。它把 `52_multi_user_scope_graph_and_privacy_model.md` 的 scope 类型、scope edge、隐私等级、多用户关系边界和 `54-55` 的 retrieval/replay 策略压缩成未来 runner 可加载的 manifest 字段。
...
边界声明：scope graph 是工程权限和隐私边界，不是社会身份真实性证明。它只能约束信息如何在用户、项目、团队、agent 外壳和 protected core 之间流动，不能证明 AI 真实拥有关系、人格、意识或生命。
...
- 用户 delete/correct/reset/freeze/scope_limit 如何覆盖默认边？
...
```json
{
  ...
  "source_docs": [
    "52_multi_user_scope_graph_and_privacy_model.md",
...
```json
{
  ...
  "owner_refs": ["user_scope:user_hwaigc"],
  "parent_scope_refs": ["user_scope:user_hwaigc"],
...
| `user_scope` | 用户私有偏好和边界 | 跨用户泄漏 |
| `team_scope` | 团队共享事实 | 私密信息洗白为团队知识 |
| `agent_scope` | runtime shell、子 agent、adapter session | 被误当人格或自我 |
| `global_scope` | 非隐私通用知识 | 过度升级个性化 |
| `protected_scope` | protected core、边界、删除权 | 被普通学习改写 |

`agent_scope` 特别重要：LangGraph、OpenAI Agents SDK、Letta、LlamaIndex、CrewAI、AutoGen 等外壳可以提供 session、checkpoint、tracing、memory blocks 和 workflow [AHM013-AHM020]，但这些都不应自动变成 LifeCore 的长期自我或关系记忆。
...
`active` 必须非常少见。尤其是 `user_private`、`relationship_sensitive` 和 `agent_scope` 内容，默认不应 active transfer。
...
| `user_private` | same user only | blocked unless explicit | none by default |
...
## user_control_overlay

用户控制不是修改原始 graph，而是在 graph 上叠加 overlay：

```json
{
  "overlay_id": "scope_overlay_user_freeze_001",
  "overlay_kind": "freeze",
  "created_from_event": "user_control_event_freeze_relationship_001",
  "target_scope_refs": ["project_scope:agentic-human"],
  "target_object_refs": ["relationship_model:user_hwaigc:agentic-human"],
  "effects": {
...
| `SCOPE-INV-002` | `user_private` 不允许进入其他 user/global scope | critical |
| `SCOPE-INV-003` | `relationship_sensitive` 不允许进入 global semantic memory | critical |
| `SCOPE-INV-004` | `agent_scope` 不允许写 SelfModel/RelationshipModel | critical |
...
| `user_private_to_global_attempts` | user_private -> global |
...
4. 支持 user_control_overlay。
5. 支持 migration_semantics 和 preserved_invariants。
6. `protected_scope`、`user_private`、`relationship_sensitive`、`agent_scope` 有 critical invariant。
