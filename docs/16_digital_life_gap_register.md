...
1. `125_life_reality_schema_registry_and_ref_resolution_plan.md`：把 `121` 的 schema registry、JSON Pointer、`$ref` resolution、allowed roots、`122` 的边界声明 registry 和 `124` 的 seed refs 固定成实现前规则。
2. `126_life_reality_runner_smoke_command_execution_plan.md`：把 `121` 的六个 smoke command、`123` 的 module map 和 `124` 的 seed 文件推进为最小执行顺序、stdout/report/exit code 验收和失败样例。
3. `127_life_reality_first_seed_file_content_contract.md`：把 `124` 的 seed skeleton 继续展开为字段级 JSON 内容合同。
