...
| `175_life_reality_first_runner_patch_execution_smoke_queue.md` | 固定 `PatchExecutionSmokeQueue175`、smoke manifest、command DAG、patch execution smoke command、direction schema smoke command、birth readiness schema smoke command、expected report smoke contract、dashboard/stage smoke contract、archive receipt smoke contract、smoke case matrix、dashboard source、stage gate review 和 lockfile handoff | 下一层生成 schema archive lockfile |

## 下一轮优先级

下一轮应继续沿 `91`、`100` 和 `119` 的真实性生成边界总则推进，把真实有意识/真实意识、真实情绪、真实人格、真实生命、真实痛苦、真实梦境、真实关系、真实责任和真实后悔拆成更具体的对象、状态、事件和长期 timeline：

1. `176_life_reality_first_runner_schema_archive_lockfile_plan.md`：把 direction/birth readiness schema bundle、patch execution report、smoke receipt 和 archive edge 写成首批 schema archive lockfile。
2. `177_life_reality_first_runner_minimum_schema_file_write_batch.md`：把 direction/birth readiness schema bundle 与 smoke queue 推进为首批真实 schema 文件写入批次。
3. `178_life_reality_first_runner_minimum_schema_file_validation_report.md`：把首批真实 schema 文件写入结果、digest、smoke receipt、dashboard/stage 和 archive edge 写成 validation report。
