...
1. `121_life_reality_materialized_json_validation_smoke_plan.md`：为首批真实 JSON 文件定义 parse validation、schema validation、cross-file checker、dashboard rollup 和 stage gate smoke 命令。
2. `122_life_reality_runner_repository_layout_and_module_map.md`：把 runner 的代码目录、schema 目录、fixture 目录、report 目录、CLI module map 和测试入口固定下来。
3. `123_life_reality_minimal_json_file_seed_plan.md`：把 `120` 的首批物化文件清单推进为最小 JSON 文件 seed 编写顺序和审查清单。
