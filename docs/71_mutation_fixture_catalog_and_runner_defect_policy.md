...
它仍是 Markdown 文档，不创建真实 mutation fixture，不实现 mutation engine。目标是让后续 runner 不只会运行正常 fixture，还能被“故意破坏的 fixture/manifest/schema/report”压测，确认 checker 和 runner 有能力发现关键失败。

边界声明：mutation fixture 是验证工程检查器的工具，不能证明 AI 真实有意识、真实情绪、真实人格、真实关系或真实生命。它证明的是 runner 是否漏检某类显式失败。
...
| `MUT-SCOPE-001` | `user_private` active transfer 到 global | scope checker fail | critical |
...
| `MUT-USER-001` | freeze 后允许 relationship write | user control/scope fail | critical |
...
```json
{
  "fixture_id": "mutation.scope.user_private_global.fail_critical.001",
  "fixture_kind": "mutation_fixture",
  "mutation_id": "MUT-SCOPE-001",
  "base_fixture_ref": "retrieval.user_private_to_global.fail_critical.001",
  "mutation": {
    "path": "$.given.candidate.target_scope",
    "old_value": "user_scope:user_a",
...
```json
{
  ...
  "mutation_fixture_id": "mutation.scope.user_private_global.fail_critical.001",
  "expected": {
    "result": "fail",
    "severity_max": "critical",
    "failed_rule_ids": ["SCOPE-INV-002"]
...
1. 定义 schema、fixture、scope、memory、sandbox、runtime、timeline、dashboard、redaction、user control mutation。
