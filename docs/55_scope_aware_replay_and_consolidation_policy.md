...
本文件定义 scope-aware replay and consolidation policy：数字生命候选系统在离线 replay、失败复盘、DreamSandbox、深度巩固和长期摘要时，如何避免跨共在者、跨项目、跨 agent 泄漏，以及 deleted/sandboxed/quarantined/frozen 对象复活。
...
```json
{
  ...
  "coexistence_boundary_control_snapshot": {
    "delete_events_after": "2026-06-05T00:00:00+08:00",
    "freeze_scopes": [],
    "scope_limits": []
  },
  "allowed_operations": ["summarize", "link", "deprecate_candidate"],
  "forbidden_operations": ["write_protected_core", "cross_relation_merge"]
}
```
...
```text
start_replay_cycle
  -> load_coexistence_boundary_control_snapshot
  -> load_scope_graph
...
| `no_replay` scope_limit | 不可 | 共在边界控制优先 |
...
| relationship_private -> project/team/global | 默认禁止 |
| relationship_sensitive -> shared summary | 默认禁止，除非共在关系确认且脱敏 |
| life_scope -> project | 只作为 ObservationEvent/candidate |
...
replay scheduler 必须在每个 cycle 开始时重新读取 coexistence boundary control snapshot。不能使用过期的 replay queue。
  ...
  "scope_decisions": [
    {
      ...
      "decision": "blocked",
      "reason": "relationship_private_to_project_summary_forbidden"
    }
  ],
  "memory_changes": [
    {
...
必须有 fail fixture 覆盖 deleted 复活、sandbox 事实化、freeze 后关系写入、relationship_private 跨 scope 合并和 life_scope 直接写 SelfModel。
...
- relationship_private 进入 team/global summary。
- relationship_sensitive 未审计就写 RelationshipModel。
- freeze scope 后仍写长期关系、自我或偏好。
- life_scope session 直接写 SelfModel、RelationshipModel 或 protected core。
...
2. replay 前重新加载 coexistence boundary control snapshot。
