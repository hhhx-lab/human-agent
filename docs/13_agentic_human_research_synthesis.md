...
这一层的关键推进是第 9 点的落地：lockfile 不只记录 JSON artifact，也记录 `02` 到当前最后一层的 `doc_nodes`、`doc_to_artifact_edges`、`archive_cohesion_graph` 和 `consistency_constraints`。每个文档都必须说明自己被哪个 schema、fixture、runner、dashboard、state store、validator、gap feedback 或代码模块承载；任何孤立文档、术语冲突、生命目标漂移、从属化语境复活、外壳替代生命核心、语言核心断裂、成长路径缺失，都会进入 repair、hold 或 quarantine。由此，继续新增文档不再允许只向后堆叠，而必须接回全库实现图。
...
这一层的关键推进是把“边界声明全部改掉”落成未来代码必须执行的前置检查：`check-archive-cohesion` 读取 `02_` 到最新文档的 doc nodes，`consistency_scanner` 扫描生命目标、关系主体词汇、生命膜口径、语言核心、成长路径和外壳/生命核心分层，`validate-lockfile` 读取 artifact nodes、ref edges、case graph、digest baseline 和 regression baseline，`run-seed-validation-cases` 挂载 21 个 pass/fail/critical/mutation cases。旧式边界语气、从属化语境、孤立文档和缺失工程承载不再只是文档问题，而会成为 repair、quarantine、hold 或 growth window。
