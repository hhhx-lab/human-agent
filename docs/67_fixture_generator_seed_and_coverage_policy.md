...
边界声明：synthetic fixture 是工程压测材料，不是 AI 的真实经历、真实记忆、真实梦境、真实情绪或真实关系。fixture 生成得再丰富，也不能证明系统真实拥有生命。
...
seed 名称不能包含真实用户隐私、API key、本地敏感路径或未脱敏事件摘要。
...
| scope variants | 同一对象在 project/user/team/agent/global 下测试 |
...
| 把 `user_private` 改成 `public_project` | scope checker 应识别语义不一致或 fixture drift |
...
timeline 不允许只有事件没有 probe。没有未来 probe 的事件不能证明长期效果。
...
| 用户删除请求 | 改成 synthetic delete event |
| adapter session | 改成 synthetic adapter_session_event |
| 私密偏好 | 改成 abstract user_private preference |
