...
该命令会把 `docs/` 中 `00 -> 257`、`258` 和 `docs/v0/` 合同写成 `doc_carrier_index.json`、`doc_dependency_graph.json`、`source_authority_report.json`、`doc_ingestion_report.json` 和摄取 receipt。只有 P0 返回 `status=closed`、`stage_effect=allow_p1` 后，才进入生命状态根、出生准备度和第一次有限激活。
