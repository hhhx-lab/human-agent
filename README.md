...
This repository uses `main` as the single active development line. The live0 implementation branch is merged back into `main`, and future commits and pushes should land on `main` unless a separate branch is explicitly requested for a temporary review flow.

If a temporary branch is created for a large experiment, close it by merging back into `main`, pushing `main`, and deleting the temporary branch after acceptance. The project should not drift into parallel long-lived branches.
...
Before the dialogue opens, the terminal renders a CRT-style birth screen: `Digital Life` and the bound life name appear around a retro terminal frame, the progress bar loads from `0.0%` to `100.0%`, and a dense upper-body humanoid is assembled from cyan/magenta binary tissue, green circuit traces, a red beating heart, and flowing particles. This screen is not a static image; it is generated by the terminal renderer and keeps moving after full load.
