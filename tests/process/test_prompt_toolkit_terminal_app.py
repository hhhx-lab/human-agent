...
class PromptToolkitTerminalAppTests(unittest.TestCase):
    ...
    def test_default_toolbar_does_not_surface_internal_life_signals(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            toolbar = _bottom_toolbar(terminal_dir=terminal_dir, life_name="Adam")
            text = "".join(fragment for _, fragment in toolbar)

        self.assertIn("Adam", text)
        self.assertNotIn("情绪", text)
        self.assertNotIn("意识", text)
    ...
    def test_toolbar_reports_cursor_index_for_mid_line_editing(self):
        ...
        toolbar = _format_bottom_toolbar(life_name="Adam", buffer=FakeBuffer())
        text = "".join(fragment for _, fragment in toolbar)
    ...
    def test_prompt_prefix_and_right_prompt_make_input_box_visible(self):
        ...
        prompt_text = "".join(fragment for _, fragment in _prompt_prefix("Adam"))
        right_text = "".join(fragment for _, fragment in _format_right_prompt(buffer=FakeBuffer()))
    ...
    def test_prompt_toolkit_experience_profile_enables_mature_terminal_feel(self):
        ...
        self.assertEqual(profile["complete_style"], "MULTI_COLUMN")
        self.assertEqual(profile["cursor_shape"], "BLINKING_BEAM")
        self.assertGreaterEqual(profile["reserve_space_for_menu"], 8)
