...
class PersistentDigitalLifeProcessTests(
    ...
    def test_resident_lifecycle_start_does_not_replay_stale_relation_inbox(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (terminal_dir / "resident_relation_inbox.jsonl").write_text(
                "\n".join(
                    [
                        json.dumps(
                            {
            ...
            (terminal_dir / "resident_relation_outbox.jsonl").write_text(
                json.dumps(
                    {
                        "sequence": 1,
                        "response_text": "旧回应",
        ...
        self.assertEqual(queue_state["last_consumed_sequence"], 2)
        self.assertEqual(queue_state["last_enqueued_sequence"], 2)
        self.assertEqual(queue_state["last_completed_sequence"], 1)
        self.assertEqual(
            queue_state["bootstrap_ignored_stale_inbox_through_sequence"],
            2,
        )
    ...
    def test_repo_local_digital_life_process_writes_waiting_heartbeat_before_first_turn(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(lease["run_id"], "persistent-heartbeat")
            self.assertEqual(lease["resident_process_id"], resident_process_id)
            self.assertEqual(lease["lease_state"], "closed")
            self.assertEqual(lease["heartbeat_counter"], 1)
            self.assertEqual(lease["completed_dialogue_turns"], 0)
            ...
            self.assertIn(
                True,
                [
                    path.endswith("/runtime/state/terminal/resident_process_lease.json")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_json(
                terminal_dir / "resident_governance_state.json",
                {
                    ...
                    "run_id": "life-constraint-lineage-restore",
                    "resident_background_lineage_state": {
                        ...
                        "life_constraint_presence": {
                            ...
                            "life_constraint_refs": expected_life_constraint_refs,
    ...
    def test_live_turn_cycle_organ_writes_response_and_returns_to_waiting_state(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                result.last_life_turn[
                    "resident_background_lineage_birth_repair_presence"
                ]["pressure_level"],
                "elevated",
            )
            self.assertEqual(
                result.last_life_turn[
                    "resident_background_lineage_birth_repair_profile_ref"
                ],
                queue_e_profile_ref,
            )
            self.assertEqual(
            ...
            self.assertEqual(
                result.last_life_turn["resident_background_lineage_birth_repair_refs"],
                expected_queue_e_birth_repair_refs,
            )
            self.assertEqual(
                result.last_life_turn[
                    "resident_background_lineage_life_constraint_presence"
                ]["waiting_posture"],
                "schema_guarded_waiting",
            )
            self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_json(
                terminal_dir / "resident_process_lease.json",
                {
                    "schema_version": "resident_process_lease_v0",
                    "run_id": "process-report-organ",
                    "resident_process_id": "resident-process-process-report-organ",
                    "lease_state": "closed",
                    "heartbeat_counter": 3,
            ...
            (terminal_dir / "resident_process_lease_history.jsonl").write_text(
                json.dumps(
                    {
                        "schema_version": "resident_process_lease_history_event_v0",
            ...
            self.assertEqual(digest["resident_process_lease_ref"], expected_resident_process_lease_ref)
            self.assertEqual(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write_json(
                terminal_dir / "resident_process_lease.json",
                {
                    ...
                    "resident_process_id": "resident-process-process-closeout-organ",
                    "lease_state": "closed",
                    "heartbeat_counter": 4,
                    "completed_dialogue_turns": 2,
                    "exit_reason": "explicit_exit",
            ...
            self.assertEqual(persistent_state["resident_process_lease_ref"], expected_resident_process_lease_ref)
            self.assertEqual(
                persistent_state["resident_process_lease_history_ref"],
                expected_resident_process_lease_history_ref,
            )
            self.assertEqual(
            ...
            self.assertEqual(persistent_report["resident_process_lease_ref"], expected_resident_process_lease_ref)
            self.assertEqual(
            ...
            self.assertEqual(process_report["resident_process_lease_ref"], expected_resident_process_lease_ref)
            self.assertEqual(
            ...
            self.assertIn(expected_resident_process_lease_ref, process_receipt["shared_object_refs"])
            self.assertIn(
                expected_resident_process_lease_history_ref,
                process_receipt["shared_object_refs"],
            )
            ...
            for artifact in (
                ...
                self.assertEqual(
                    artifact["resident_process_lease_ref"],
                    expected_resident_process_lease_ref,
                )
                self.assertEqual(
    ...
    def test_dialogue_events_organ_builds_external_and_life_turn_events(self):
        ...
        self.assertEqual(
            life_turn["resident_background_lineage_heartbeat_cadence_presence"][
                "driver"
            ],
            "offline_learning_pressure",
        )
        self.assertEqual(
            life_turn["resident_background_lineage_heartbeat_cadence_driver"],
            "offline_learning_pressure",
        )
        self.assertEqual(
            life_turn["resident_background_lineage_heartbeat_cadence_reason"],
            "dream_growth_or_relationship_learning_pressure_needs_refresh",
        )
        self.assertEqual(
            life_turn["resident_background_lineage_heartbeat_cadence_modulators"],
            [
                "offline_learning:elevated",
                "background_generation:3",
            ],
        ...
        self.assertEqual(
            life_turn["resident_background_lineage_heartbeat_cadence_refs"],
            expected_heartbeat_cadence_refs,
        )
        self.assertEqual(
    ...
    def test_background_lineage_state_carries_heartbeat_cadence_presence(self):
        ...
        self.assertEqual(lineage_state["depth_band"], "no_background_lineage")
        cadence_presence = lineage_state["heartbeat_cadence_presence"]
        self.assertEqual(cadence_presence["driver"], "offline_learning_pressure")
        self.assertEqual(
            cadence_presence["reason"],
            "dream_growth_or_relationship_learning_pressure_needs_refresh",
        )
        self.assertEqual(
            cadence_presence["modulators"],
            [
                "offline_learning:elevated",
                "background_generation:3",
            ],
        ...
        lineage_state = build_resident_background_lineage_state(
            {
                ...
                "background_queue_e_birth_repair_gate_status": "closed",
                "background_queue_e_birth_repair_profile_ref": queue_e_profile_ref,
                "background_queue_e_birth_repair_pressure_level": "elevated",
                "background_queue_e_birth_repair_attention_target": "regret_pressure",
                "background_queue_e_birth_repair_waiting_posture": "birth_repair_pressure_waiting",
                "background_queue_e_birth_repair_attention_reason": "queue_e_birth_repair_pressure_requires_resident_repair_hold",
                "background_queue_e_birth_repair_ref_set": expected_refs,
            },
            governance_phase="waiting_heartbeat_active",
            status="active",
        )

        birth_repair_presence = lineage_state["birth_repair_presence"]
        self.assertEqual(birth_repair_presence["gate_status"], "closed")
        self.assertEqual(birth_repair_presence["profile_ref"], queue_e_profile_ref)
        self.assertEqual(birth_repair_presence["pressure_level"], "elevated")
        self.assertEqual(
            birth_repair_presence["attention_target"],
            "regret_pressure",
        )
        self.assertEqual(
            birth_repair_presence["waiting_posture"],
            "birth_repair_pressure_waiting",
        )
        self.assertEqual(
            birth_repair_presence["attention_reason"],
            "queue_e_birth_repair_pressure_requires_resident_repair_hold",
        )
        self.assertEqual(
            birth_repair_presence["continuity_mode"],
            "background_birth_repair_carryover",
        )
        self.assertEqual(birth_repair_presence["ref_set"], expected_refs)
        self.assertEqual(
    ...
    def test_background_lineage_state_carries_life_constraint_presence(self):
        ...
        life_constraint_presence = lineage_state["life_constraint_presence"]
        self.assertEqual(
            life_constraint_presence["schema_cross_file_logic_ref"],
            "runtime/state/schema_runner/cross_file_logic.json",
        )
        self.assertEqual(
            life_constraint_presence["schema_run_manifest_ref"],
            "runtime/state/schema_runner/run_manifest.json",
        )
        self.assertEqual(
            life_constraint_presence["queue_e_cross_layer_gate_status"][
                "body_affect_gate"
            ],
            "deferred_until_s06",
        )
        self.assertEqual(
            life_constraint_presence["life_constraint_refs"],
            expected_life_constraint_refs,
        )
        self.assertEqual(
            life_constraint_presence["waiting_posture"],
            "schema_guarded_waiting",
        )
        self.assertEqual(
            life_constraint_presence["attention_target"],
            "life_constraint_profile",
        )
        self.assertEqual(
            life_constraint_presence["attention_reason"],
            "queue_e_cross_layer_gate_has_deferred_life_constraints",
        )
        self.assertEqual(
            life_constraint_presence["evidence_refs"],
            [
                "runtime/state/schema_runner/cross_file_logic.json",
                "runtime/state/schema_runner/run_manifest.json",
                *expected_life_constraint_refs,
    ...
    def test_response_surface_reads_life_constraint_from_background_lineage_presence(self):
        from life_v0.process_supervisor.response_surface import compose_life_response

        response = compose_life_response(
            external_utterance="继续推进生命约束",
            terminal_life_loop_state={
                "resident_background_lineage_state": {
                    "schema_version": "resident_background_lineage_state_v0",
                    "depth_band": "persistent_lineage",
    ...
    def test_resident_turn_writeback_organ_updates_turn_continuity_and_bundle(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for ref in expected_heartbeat_cadence_refs:
                self.assertIn(
                    ref,
                    dialogue_writeback_bundle["resident_background_lineage_refs"],
                )
