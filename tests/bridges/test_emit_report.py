...
class EmitReportTests(unittest.TestCase):
    ...
    def test_run_emit_report_writes_first_activation_return_bundle(self):
        ...
        self.assertEqual(bundle["queue_e_state_refs"][0], "runtime/state/action/responsibility_loop_state.json")
        self.assertEqual(bundle["queue_e_report_refs"], ["runtime/reports/latest/pain_regret_repair_report.json"])
        self.assertEqual(bundle["world_contact_release_posture"], "shadow_only_guarded")
        self.assertTrue(bundle["repair_followup_required"])

        self.assertEqual(bundle_digest["schema_version"], "report_bundle_digest_v0")
        self.assertEqual(bundle_digest["status"], "closed")
        self.assertEqual(bundle_digest["queue_e_ref_count"], 3)
        ...
        self.assertEqual(return_packet["responsibility_restore_refs"], ["runtime/state/action/responsibility_loop_state.json"])
        self.assertEqual(return_packet["world_contact_restore_refs"], ["runtime/state/membrane/world_contact_summary.json"])
        self.assertEqual(return_packet["pain_regret_restore_refs"], ["runtime/reports/latest/pain_regret_repair_report.json"])
        self.assertTrue(return_packet["regret_pressure_restore_refs"])
        self.assertTrue(return_packet["repair_obligation_restore_refs"])
        self.assertTrue(return_packet["responsibility_language_restore_refs"])
        self.assertEqual(return_packet["world_contact_release_posture"], "shadow_only_guarded")
        self.assertTrue(return_packet["repair_followup_required"])
