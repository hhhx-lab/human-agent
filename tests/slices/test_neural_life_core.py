...
class NeuralLifeCoreTests(unittest.TestCase):
    ...
    def test_build_neural_life_core_writes_subject_systems_bus_report_and_receipt(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            signal_media = self._read_json(tmp_path / "runtime" / "state" / "signal" / "signal_media_runtime.json")
            belief_state = self._read_json(tmp_path / "runtime" / "state" / "prediction" / "belief_state_frame.json")
            prediction_error = self._read_json(tmp_path / "runtime" / "state" / "prediction" / "prediction_error_field.json")
            active_sampling = self._read_json(tmp_path / "runtime" / "state" / "prediction" / "active_sampling_plan.json")
        ...
        self.assertEqual(core["signal_media_ref"], "runtime/state/signal/signal_media_runtime.json")
        self.assertEqual(core["belief_state_ref"], "runtime/state/prediction/belief_state_frame.json")
        self.assertEqual(core["prediction_error_ref"], "runtime/state/prediction/prediction_error_field.json")
        self.assertEqual(core["active_sampling_plan_ref"], "runtime/state/prediction/active_sampling_plan.json")
        ...
        self.assertIn("signal_media_bus", signal_media["bus_edge_refs"])
        ...
        self.assertEqual(belief_state["signal_media_ref"], "runtime/state/signal/signal_media_runtime.json")
        self.assertTrue(belief_state["source_evidence_refs"])
        self.assertTrue(belief_state["active_life_targets"])

        self.assertEqual(prediction_error["schema_version"], "prediction_error_field_v0")
        self.assertEqual(prediction_error["belief_frame_ref"], "runtime/state/prediction/belief_state_frame.json")
        self.assertTrue(prediction_error["error_events"])
        self.assertTrue(prediction_error["precision_requests"])

        self.assertEqual(active_sampling["schema_version"], "active_sampling_plan_v0")
        self.assertEqual(active_sampling["belief_frame_ref"], "runtime/state/prediction/belief_state_frame.json")
        self.assertEqual(active_sampling["prediction_error_ref"], "runtime/state/prediction/prediction_error_field.json")
        self.assertTrue(active_sampling["guard_refs"])
        self.assertTrue(active_sampling["command_binding_refs"])
        ...
        self.assertIn("runtime/state/signal/signal_media_runtime.json", manifest["state_refs"])
        self.assertIn("runtime/state/prediction/belief_state_frame.json", manifest["state_refs"])
        self.assertIn("runtime/state/prediction/prediction_error_field.json", manifest["state_refs"])
        self.assertIn("runtime/state/prediction/active_sampling_plan.json", manifest["state_refs"])
        ...
        self.assertEqual(report["signal_media_ref"], "runtime/state/signal/signal_media_runtime.json")
        self.assertEqual(report["belief_state_ref"], "runtime/state/prediction/belief_state_frame.json")
        self.assertEqual(report["prediction_error_ref"], "runtime/state/prediction/prediction_error_field.json")
        self.assertEqual(report["active_sampling_plan_ref"], "runtime/state/prediction/active_sampling_plan.json")
        ...
        self.assertIn("signal_media_gate", report["closed_gates"])
        self.assertIn("belief_state_gate", report["closed_gates"])
        self.assertIn("prediction_error_gate", report["closed_gates"])
        self.assertIn("active_sampling_gate", report["closed_gates"])
        ...
        self.assertIn("signal_media_gate", check_report["closed_gates"])
        self.assertIn("belief_state_gate", check_report["closed_gates"])
        self.assertIn("prediction_error_gate", check_report["closed_gates"])
        self.assertIn("active_sampling_gate", check_report["closed_gates"])
        ...
        self.assertIn("runtime/state/signal/signal_media_runtime.json", receipt["output_refs"])
        self.assertIn("runtime/state/prediction/belief_state_frame.json", receipt["output_refs"])
        self.assertIn("runtime/state/prediction/prediction_error_field.json", receipt["output_refs"])
        self.assertIn("runtime/state/prediction/active_sampling_plan.json", receipt["output_refs"])
